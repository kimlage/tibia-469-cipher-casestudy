#!/usr/bin/env python3
"""Completion audit for human translation atlas v4."""
...
def ensure_tables(conn: sqlite3.Connection) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS human_translation_completion_audit_v3_runs (
            run_id INTEGER PRIMARY KEY AUTOINCREMENT,
            created_at TEXT NOT NULL,
            decision TEXT NOT NULL,
            atlas_v4_run_id INTEGER NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS human_translation_completion_audit_v3_missing_books (
...
def reason_for(tags: str) -> tuple[str, str]:
    ...
    if "R20_R02_PHASE_FRAME" in tags:
        return ("R5_PLAUSIBLE_PROSE_SHADOW", "R20/R02 residual phase family still missing atlas coverage")
    if "C86_PAYLOAD_OPERATOR" in tags or "VNCTIIN_CONTEXT_FRAME" in tags:
        return ("R2_NPC_PHRASE_STYLE_COMPARATOR + R5_PLAUSIBLE_PROSE_SHADOW", "C86/VNCTIIN residual family still missing atlas coverage")
...
def main() -> None:
    ...
    atlas_run_id = int(conn.execute("SELECT max(run_id) AS run_id FROM human_translation_atlas_v4_items").fetchone()["run_id"])
    books = conn.execute(
        """
    ...
    atlas = {
        ...
        for row in conn.execute(
            "SELECT target_id, promotion_status FROM human_translation_atlas_v4_items WHERE run_id=?",
            (atlas_run_id,),
        ).fetchall()
    }
    ...
    decision = "HUMAN_TRANSLATION_OBJECTIVE_NOT_COMPLETE_ATLAS_V4_EXPAND_BENNA_NAESE_RESIDUALS"
    blocker_summary = {
        "current_strength": "40 anchored shadow readings after adding R20/R02 phase family",
        "remaining_priority": ["unassigned residuals", "BENNA/formula", "NAESE/C68 slot"],
    ...
    cur = conn.execute(
        """
        INSERT INTO human_translation_completion_audit_v3_runs
        (created_at, decision, atlas_v4_run_id, total_book_count,
    ...
    for row in missing:
        ...
        conn.execute(
            """
            INSERT INTO human_translation_completion_audit_v3_missing_books
            ...
            (
                ...
                route,
                reason,
                "seed the next family only after bridge/probe split exists",
            ),
        )
