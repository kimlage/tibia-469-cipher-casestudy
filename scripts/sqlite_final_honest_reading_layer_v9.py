...
def create(cur):
    cur.executescript('''
    create table if not exists final_honest_reading_v9_runs(
      run_id integer primary key autoincrement, created_at text not null, decision text not null,
      source_v8_run_id integer not null, source_book30_gate_run_id integer not null,
    ...
    create table if not exists final_honest_reading_v9_books(
      run_id integer not null, bookid text not null, reading_status text not null, audit_covered integer not null,
      gloss_allowed integer not null, functional_tag_count integer not null, added_v9_tag_count integer not null,
...
def main():
    ...
    v8=one(cur,'select max(run_id) as run_id from final_honest_reading_v8_books')['run_id']
    gate=one(cur,'select max(run_id) as run_id from book30_core_context_gate_items')['run_id']
    books=rows(cur,'select * from final_honest_reading_v8_books where run_id=? order by cast(bookid as integer)',(v8,))
    gitems={}
    for x in rows(cur,"select * from book30_core_context_gate_items where run_id=? and decision='BOOK30_CORE_CONTEXT_NO_GLOSS'",(gate,)):
    ...
    for b in books:
        ...
        for x in gitems.get(bookid,[]):
            add(tags,'BOOK30_CORE_CONTEXT',x['functional_label'],'book30_core_context_gate_items',float(x['confidence']))
            additions.append({'gate':'BOOK30_CORE','segment_id':x['segment_id'],'label':x['functional_label']})
        added=len(tags)-before; status='FUNCTIONALLY_TAGGED_NO_GLOSS' if tags else b['reading_status']; prefix=''
        if added: prefix='<V9_FUNCTIONAL:'+'; '.join(a['label'] for a in additions)+'> '
        ev=json.loads(b['evidence_json'] or '{}'); ev['v9_added_tags']=additions
    ...
    cur.execute('insert into final_honest_reading_v9_runs(created_at,decision,source_v8_run_id,source_book30_gate_run_id,book_count,audit_covered_book_count,functional_tagged_book_count,newly_tagged_book_count,semantic_gloss_allowed_count,operational_coverage_pct,functional_tagged_book_pct,semantic_gloss_pct,payload_json) values (?,?,?,?,?,?,?,?,?,?,?,?,?)',
        (now(),'FINAL_HONEST_READING_V9_BOOK30_CORE_FUNCTIONAL_TAGS_NO_SEMANTIC_TRANSLATION',v8,gate,n,audit,tagged,newly,0,op,fp,0.0,j({'note':'v9 adds Book30 core context tags only'})))
    run_id=cur.lastrowid
    for x in out:
        cur.execute('insert into final_honest_reading_v9_books(run_id,bookid,reading_status,audit_covered,gloss_allowed,functional_tag_count,added_v9_tag_count,functional_tags_json,honest_text,evidence_json) values (?,?,?,?,?,?,?,?,?,?)',(run_id,)+x)
    con.commit(); res={'run_id':run_id,'decision':'FINAL_HONEST_READING_V9_BOOK30_CORE_FUNCTIONAL_TAGS_NO_SEMANTIC_TRANSLATION','book_count':n,'functional_tagged_book_count':tagged,'newly_tagged_book_count':newly,'functional_tagged_book_pct':fp,'semantic_gloss_pct':0.0}; print(json.dumps(res,ensure_ascii=False))
    if args.discord:
        send('\n'.join([f"[469][honest-reading-v9][run={run_id}] Book30-core incorporado",f"livros={n} | função auditável={tagged}/{n} ({fp}%) | novos na v9={newly} | gloss lexical=0%",'camada v9 aumenta leitura funcional honesta sem inventar significado lexical.']))
