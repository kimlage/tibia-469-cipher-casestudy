#!/usr/bin/env python3
"""Build human translation atlas v6 by adding residual shadow readings."""
...
def ensure_tables(conn: sqlite3.Connection) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS human_translation_atlas_v6_runs (
            run_id INTEGER PRIMARY KEY AUTOINCREMENT,
            created_at TEXT NOT NULL,
            decision TEXT NOT NULL,
            atlas_v5_run_id INTEGER NOT NULL,
            residual_shadow_run_id INTEGER NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS human_translation_atlas_v6_items (
...
def main() -> None:
    ...
    atlas_v5_run_id = max_id(conn, "human_translation_atlas_v5_items")
    residual_shadow_run_id = max_id(conn, "human_residual_shadow_v1_items")
    prepared: list[dict[str, object]] = [
        dict(row)
        for row in conn.execute(
            "SELECT * FROM human_translation_atlas_v5_items WHERE run_id=?",
            (atlas_v5_run_id,),
        ).fetchall()
    ]
    new_items = []
    for row in conn.execute(
        "SELECT * FROM human_residual_shadow_v1_items WHERE run_id=?",
        (residual_shadow_run_id,),
    ).fetchall():
        item = {
            "target_id": str(row["bookid"]),
            "target_kind": "book",
            "source_layer": "human_residual_shadow_v1",
            ...
            "probe_evidence": "human_residual_shadow_v1",
    ...
    decision = "HUMAN_TRANSLATION_ATLAS_V6_READY_70_SHADOW_READINGS"
    cur = conn.execute(
        """
        INSERT INTO human_translation_atlas_v6_runs
        (created_at, decision, atlas_v5_run_id, residual_shadow_run_id,
        ...
        (
            ...
            ready,
            promoted,
            json.dumps({"principle": "v6 adds residual shadow readings for full book coverage"}, ensure_ascii=False, sort_keys=True),
        ),
    )
    ...
    for item in final_items:
        conn.execute(
            """
            INSERT INTO human_translation_atlas_v6_items
