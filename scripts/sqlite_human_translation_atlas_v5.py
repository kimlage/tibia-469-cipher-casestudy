#!/usr/bin/env python3
"""Build human translation atlas v5 by adding slot/formula readings."""
...
def ensure_tables(conn: sqlite3.Connection) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS human_translation_atlas_v5_runs (
            run_id INTEGER PRIMARY KEY AUTOINCREMENT,
            created_at TEXT NOT NULL,
            decision TEXT NOT NULL,
            atlas_v4_run_id INTEGER NOT NULL,
            slot_formula_shadow_run_id INTEGER NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS human_translation_atlas_v5_items (
...
def main() -> None:
    ...
    atlas_v4_run_id = max_id(conn, "human_translation_atlas_v4_items")
    slot_formula_shadow_run_id = max_id(conn, "human_slot_formula_shadow_v1_items")
    prepared: list[dict[str, object]] = [
        dict(row)
        for row in conn.execute(
            "SELECT * FROM human_translation_atlas_v4_items WHERE run_id=?",
            (atlas_v4_run_id,),
        ).fetchall()
    ]
    new_items = []
    for row in conn.execute(
        "SELECT * FROM human_slot_formula_shadow_v1_items WHERE run_id=?",
        (slot_formula_shadow_run_id,),
    ).fetchall():
        item = {
            "target_id": str(row["bookid"]),
            "target_kind": "book",
            "source_layer": "human_slot_formula_shadow_v1",
            ...
            "probe_evidence": "human_slot_formula_shadow_v1",
    ...
    decision = "HUMAN_TRANSLATION_ATLAS_V5_READY_51_SHADOW_READINGS"
    cur = conn.execute(
        """
        INSERT INTO human_translation_atlas_v5_runs
        (created_at, decision, atlas_v4_run_id, slot_formula_shadow_run_id,
        ...
        (
            ...
            ready,
            promoted,
            json.dumps({"principle": "v5 adds BENNA/formula and NAESE/C68 slot shadow readings"}, ensure_ascii=False, sort_keys=True),
        ),
    )
    ...
    for item in final_items:
        conn.execute(
            """
            INSERT INTO human_translation_atlas_v5_items
