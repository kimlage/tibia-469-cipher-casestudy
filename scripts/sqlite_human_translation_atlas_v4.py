#!/usr/bin/env python3
"""Build human translation atlas v4 by adding R20/R02 phase readings."""
...
def ensure_tables(conn: sqlite3.Connection) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS human_translation_atlas_v4_runs (
        ...
        CREATE TABLE IF NOT EXISTS human_translation_atlas_v4_items (
...
def main() -> None:
    ...
    atlas_v3_run_id = max_id(conn, "human_translation_atlas_v3_items")
    r20_r02_shadow_run_id = max_id(conn, "human_r20_r02_phase_shadow_v1_items")
    prepared: list[dict[str, object]] = [
        dict(row)
        for row in conn.execute(
            "SELECT * FROM human_translation_atlas_v3_items WHERE run_id=?",
            (atlas_v3_run_id,),
        ).fetchall()
    ]
    new_items = []
    for row in conn.execute(
        "SELECT * FROM human_r20_r02_phase_shadow_v1_items WHERE run_id=?",
        (r20_r02_shadow_run_id,),
    ).fetchall():
        item = {
            "target_id": str(row["bookid"]),
            "target_kind": "book",
            "source_layer": "human_r20_r02_phase_shadow_v1",
            ...
            "probe_evidence": "human_r20_r02_phase_shadow_v1",
    ...
    decision = "HUMAN_TRANSLATION_ATLAS_V4_READY_40_SHADOW_READINGS"
    cur = conn.execute(
        """
        INSERT INTO human_translation_atlas_v4_runs
        (created_at, decision, atlas_v3_run_id, r20_r02_shadow_run_id,
        ...
        (
            ...
            ready,
            promoted,
            json.dumps({"principle": "v4 adds R20/R02 phase-family shadow readings"}, ensure_ascii=False, sort_keys=True),
        ),
    )
    ...
    for item in final_items:
        conn.execute(
            """
            INSERT INTO human_translation_atlas_v4_items
