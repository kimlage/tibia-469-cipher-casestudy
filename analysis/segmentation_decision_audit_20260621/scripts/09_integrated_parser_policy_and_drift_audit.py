...
OUT_STEM = "09_integrated_parser_policy_and_drift_audit"
...
def predict_offset(
    rows: list[dict[str, int]], policy: str, field: str | None, confirm_window: int | None
) -> int | None:
    if policy == "first_match":
        for row in rows:
            if row["max_copy_length"] >= MIN_COPY_LEN:
                return row["offset"]
        return None
    if policy != "first_confirmed_peak":
        raise ValueError(policy)
    if field is None or confirm_window is None:
        raise ValueError("confirmed peak policy requires field and confirm_window")
...
def parse_book(
    ...
    while pos < len(target):
        ...
        predicted_offset = predict_offset(
            rows,
            policy["policy"],
            policy.get("field"),
            policy.get("confirm_window"),
        )
...
def policy_label(policy: dict[str, Any]) -> str:
    if policy["policy"] == "first_match":
        return "first_match"
    return f"{policy['field']}:window{policy['confirm_window']}"
...
def score_policy(
    ...
    for book in range(10, 70):
        predicted, emitted = parse_book(trace_module, books[book], emitted, policy)
    ...
    return {
        ...
        "confirm_window": policy.get("confirm_window"),
...
def prequential_policy_selection(scores: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows = []
...
def make_policies() -> list[dict[str, Any]]:
    policies: list[dict[str, Any]] = [{"policy": "first_match"}]
    for field in ["max_copy_length", "total_advance"]:
...
def make_result() -> dict[str, Any]:
    integrated = load_json(INTEGRATED_ONLINE)
    assert_boundary("integrated_online_literal_parser_audit", integrated)

    trace_module = load_module("segmentation_trace_for_gate09", TRACE_SCRIPT)
    gate111 = load_module("gate111_for_segmentation_gate09", GATE111_SCRIPT)
    ...
    scores = [score_policy(trace_module, books, stable_by_book, policy) for policy in policies]
    best = max(
        scores,
        key=lambda row: (
            row["exact_book_count"],
    ...
    active_label = "max_copy_length:window6"
    active = next(row for row in scores if row["label"] == active_label)
    first_match = next(row for row in scores if row["label"] == "first_match")
    preq = prequential_policy_selection(scores)

    exact_improvement_vs_active = best["exact_book_count"] - active["exact_book_count"]
    prequential_stable = all(row["selected_matches_oracle"] for row in preq)
    promotes_integrated_policy = (
        best["exact_book_count"] == best["tested_books"]
        and prequential_stable
    )
    if promotes_integrated_policy:
        classification = "integrated_parser_policy_promoted_target_text_parser"
    elif exact_improvement_vs_active > 0 and prequential_stable:
        classification = "integrated_parser_policy_prequential_partial_not_promoted"
    elif exact_improvement_vs_active > 0:
        classification = "integrated_parser_policy_posthoc_improvement_not_promoted"
    else:
        classification = "integrated_parser_policy_frontier_no_improvement"

    return {
        "schema": "integrated_parser_policy_and_drift_audit.v1",
        "classification": classification,
        "translation_delta": "NONE",
        "case_reopened": False,
        "plaintext_claim": False,
        "inputs": {
            "integrated_online_literal_parser_audit": rel(INTEGRATED_ONLINE),
        ...
        "summary": {
            ...
            "active_policy": active_label,
            "active_exact_books": active["exact_book_count"],
            "first_match_exact_books": first_match["exact_book_count"],
            ...
            "exact_improvement_vs_active": exact_improvement_vs_active,
            ...
            "promotes_integrated_policy": promotes_integrated_policy,
            "interpretation": (
                "Retuning the simple local-peak stop family from confirmation "
                "window 6 to 5 is prefix-stable and reduces drift, but it still "
                "does not remove integrated parser drift or reach exact stable "
                "projection coverage."
            ),
        },
        "policy_scoreboard": [
            ...
            for row in sorted(scores, key=lambda item: (-item["exact_book_count"], item["label"]))
        ],
        "active_mismatch_rows": active["mismatch_rows"],
...
def write_result(result: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- First-match exact books: `{s['first_match_exact_books']}/60`.",
        f"- Active gate-08 policy `{s['active_policy']}` exact books: `{s['active_exact_books']}/60`.",
        f"- Best full-corpus policy `{s['best_policy']}` exact books: `{s['best_exact_books']}/60`.",
        f"- Exact-book improvement vs active: `{s['exact_improvement_vs_active']}`.",
        "",
        "| Policy | Exact books | Literal gaps | Literal digits | Drift classes |",
        "|---|---:|---:|---:|---|",
    ]
    for row in result["policy_scoreboard"][:12]:
        lines.append(
    ...
    lines.extend(
        [
            ...
            "",
            f"- Active mismatch books: `{[row['book'] for row in result['active_mismatch_rows']]}`.",
            f"- Active drift classes: `{dict(Counter(row['drift_class'] for row in result['active_mismatch_rows']))}`.",
            "",
            "| Book | Class | First diff |",
            "|---:|---|---|",
        ]
    )
    for row in result["active_mismatch_rows"]:
        lines.append(
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            f"- Promotes integrated parser policy: `{s['promotes_integrated_policy']}`.",
            f"- {s['interpretation']}",
