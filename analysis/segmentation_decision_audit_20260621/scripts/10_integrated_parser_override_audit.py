...
OUT_STEM = "10_integrated_parser_override_audit"
...
def predict_window5(rows: list[dict[str, int]]) -> int | None:
...
def override_applies(policy: dict[str, Any], pos: int, copy: dict[str, int] | None) -> bool:
    if copy is None:
        return False
    if copy["length"] < policy["immediate_min_len"]:
        return False
    scope = policy["override_scope"]
    if scope == "none":
        return False
    if scope == "book_start":
        return pos == 0
...
def parse_book(
    ...
    while pos < len(target):
        ...
        if override_applies(policy, pos, immediate_copy):
            ops.append(
                {
                    "type": "copy",
                    "target_start": pos,
                    "length": immediate_copy["length"],
                    "source": immediate_copy["source"],
                }
            )
            emitted += target[pos : pos + immediate_copy["length"]]
            pos += immediate_copy["length"]
            continue

        rows = offset_rows(trace_module, emitted, target, pos)
        predicted_offset = predict_window5(rows)
...
def policy_label(policy: dict[str, Any]) -> str:
    if policy["override_scope"] == "none":
        return "window5:no_override"
    return f"window5:{policy['override_scope']}_immediate_len{policy['immediate_min_len']}"
...
def score_policy(
    ...
    return {
        ...
        "override_scope": policy["override_scope"],
        "immediate_min_len": policy["immediate_min_len"],
        "tested_books": 60,
...
def make_policies() -> list[dict[str, Any]]:
    policies = [{"override_scope": "none", "immediate_min_len": 10**9}]
    for scope in ["book_start", "internal", "any"]:
        for threshold in THRESHOLDS:
            policies.append(
                {
                    "override_scope": scope,
                    "immediate_min_len": threshold,
                }
            )
    return policies
...
def make_result() -> dict[str, Any]:
    ...
    if policy_drift["summary"]["best_policy"] != "max_copy_length:window5":
        raise RuntimeError("gate09 best policy changed")

    trace_module = load_module("segmentation_trace_for_gate10", TRACE_SCRIPT)
    gate111 = load_module("gate111_for_segmentation_gate10", GATE111_SCRIPT)
    ...
    baseline = next(row for row in scores if row["label"] == "window5:no_override")
    best = max(
        scores,
        key=lambda row: (
            row["exact_book_count"],
    ...
    promotes_override_policy = (
        best["exact_book_count"] == 60 and prequential_stable
    )
    if promotes_override_policy:
        classification = "integrated_override_policy_promoted_target_text_parser"
    elif exact_improvement > 0 and prequential_stable:
        classification = "integrated_override_policy_prequential_partial_not_promoted"
    elif exact_improvement > 0:
        classification = "integrated_override_policy_posthoc_improvement_not_promoted"
    else:
        classification = "integrated_override_policy_rejected"

    return {
        "schema": "integrated_parser_override_audit.v1",
        ...
        "summary": {
            ...
            "promotes_override_policy": promotes_override_policy,
            "interpretation": (
                "Immediate-copy overrides test whether residual window5 drift is "
                "mainly a missed-copy problem. The family is promoted only if it "
                "reaches exact stable projection coverage under prefix selection."
            ),
        },
        "policy_scoreboard": [
            {
                ...
                "immediate_min_len": row["immediate_min_len"],
...
def write_result(result: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            f"- Promotes override policy: `{s['promotes_override_policy']}`.",
            f"- {s['interpretation']}",
