...
| ES/LATAM / Spanish | Duplicate inventory and fan-claim pointers; eye-count variant already covered. |
| DE/other / German | No promoted/official German ground truth; fan vocabulary rejected. |
...
Additional closure fronts:

| Front | Disposition |
|---|---|
| Tridiag / incomplete formula | `weak_clue`; E/diagonal pressure remains local and non-generative. |
| Donina / red light / controller | `weak_clue`; render/zero metadata only. |
| Paradox / mirror books | `rejected_control`; false-positive calibration. |
| Spirit Grounds / Gate Keeper | `rejected_control`; weird language plus gate is insufficient. |
| Evil Mastermind fake dictionaries | `rejected_control`; process guard against dictionary overfit. |
| Dreadeye | `watchlist_only`; no direct formula target. |
| First Dragon | `watchlist_only`; official-source gate only. |
