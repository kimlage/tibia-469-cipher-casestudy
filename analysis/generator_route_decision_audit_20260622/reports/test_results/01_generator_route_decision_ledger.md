...
| --- | --- | --- | --- |
| `current_executable_decoder` | `PROMOTED_BASELINE_NOT_FINAL_GENERATOR` | `False` | baseline is executable and promoted, but event policy and non-continuation source/length remain external |
...
| `nonlocal_event_policy_program` | `OPEN_INTERNAL_ROUTE` | `True` | must generate replay event schedule and copy/literal/source-length choices in prefix/family holdout, or reduce declared external fields after paying program/corrections |
