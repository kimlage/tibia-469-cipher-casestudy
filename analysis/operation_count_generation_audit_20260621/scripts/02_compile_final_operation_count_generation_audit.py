...
GATE = TEST_RESULTS / "01_operation_count_generation_gate.json"
OUT = REPORTS / "final_operation_count_generation_audit.md"
...
def main() -> None:
    gate = load_json(GATE)
    assert_boundary("operation_count_generation_gate", gate)
    s = gate["summary"]
    classification = (
        "OPERATION_COUNT_GENERATOR_PROMOTED"
        if s["promotes_operation_count_generator"]
        else "OPERATION_COUNT_GENERATOR_REJECTED_WITH_AUDIT_ONLY_CONTEXT_CLUE"
    )
    lines = [
