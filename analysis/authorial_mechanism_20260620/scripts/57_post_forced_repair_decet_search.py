...
def main() -> None:
    ...
    best_decet = None
    compatible_decets = 0
    incompatible_decets = 0
    for repairs in itertools.combinations(candidates, 10):
        if not repairs_mutually_compatible(pair_module, repairs):
            incompatible_decets += 1
            continue
        compatible_decets += 1
        ...
        if best_decet is None or row["total_bits"] < best_decet["total_bits"]:
            best_decet = row

    best_single = min(candidates, key=lambda row: row["single_total_bits"]) if candidates else None
    promoted = best_decet is not None and best_decet["total_bits"] < current_bits
    classification = (
        "post_forced_repair_decet_improvement"
        if promoted
        else "post_forced_repair_decet_not_promoted"
    )

    result = {
        "schema": "post_forced_repair_decet_search.v1",
        "test": "57_post_forced_repair_decet_search",
        ...
        "compatible_decets_tested": compatible_decets,
        "incompatible_decets_skipped": incompatible_decets,
        "total_decets_considered": compatible_decets + incompatible_decets,
        "best_single_repair": best_single,
        "best_decet_repair": best_decet,
    ...
    lines = [
        ...
        f"| Compatible decets tested | `{compatible_decets}` |",
        f"| Incompatible decets skipped | `{incompatible_decets}` |",
    ]
    if best_single is not None:
        lines.append(f"| Best single delta | `{best_single['single_delta_vs_current_bits']:.1f}` |")
    if best_decet is not None:
        lines.extend(
            [
                f"| Best decet total bits | `{best_decet['total_bits']:.1f}` |",
                f"| Best decet delta vs current | `{best_decet['delta_vs_current_bits']:.1f}` |",
            ]
        )
    lines.extend(
        [
    ...
    for row in (best_decet or {"repairs": []})["repairs"]:
        lines.append(
    ...
    write_result("57_post_forced_repair_decet_search", result, lines)
