...
def best_seed_address(row: dict, literal_rows: list[dict]) -> dict | None:
    available = [literal for literal in literal_rows if literal["global_end"] <= row["target_global"]]
    candidates = []
...
def main() -> None:
    ...
    fixed_nonaddress_bits = current_bits - absolute_bits
    ...
    for row in [*standard, *seed_models]:
        total_bits = fixed_nonaddress_bits + row["copy_bits"]
    ...
    if best_decodable["model"] != "absolute_flat_source_pos" and best_decodable["total_bits"] < current_bits:
        classification = "current_formula_address_model_candidate"
    elif best_any["total_bits"] < current_bits and not best_any["decodable"]:
        classification = "current_formula_address_optimistic_only_not_promoted"
    else:
        classification = "current_formula_address_absolute_retained"

    result = {
        "schema": "current_formula_address_model_search.v1",
        "test": "28_current_formula_address_model_search",
        "classification": classification,
        "translation_delta": "NONE",
        "source_formula": str(FORMULA.relative_to(ROOT)),
        "current_formula_bits": current_bits,
        "fixed_nonaddress_bits": fixed_nonaddress_bits,
    ...
    write_result("28_current_formula_address_model_search", result, lines)
