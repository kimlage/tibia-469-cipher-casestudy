...
def main() -> None:
    ...
    best_sept = None
    compatible_septs = 0
    incompatible_septs = 0
    for repairs in itertools.combinations(candidates, 7):
        if not repairs_mutually_compatible(pair_module, repairs):
            incompatible_septs += 1
            continue
        compatible_septs += 1
        ...
        if best_sept is None or row["total_bits"] < best_sept["total_bits"]:
            best_sept = row

    best_single = min(candidates, key=lambda row: row["single_total_bits"]) if candidates else None
    promoted = best_sept is not None and best_sept["total_bits"] < current_bits
    classification = (
        "post_forced_repair_sept_improvement"
        if promoted
        else "post_forced_repair_sept_not_promoted"
    )

    result = {
        "schema": "post_forced_repair_sept_search.v1",
        "test": "54_post_forced_repair_sept_search",
        ...
        "compatible_septs_tested": compatible_septs,
        "incompatible_septs_skipped": incompatible_septs,
        "total_septs_considered": compatible_septs + incompatible_septs,
        "best_single_repair": best_single,
        "best_sept_repair": best_sept,
    ...
    lines = [
        ...
        f"| Compatible septets tested | `{compatible_septs}` |",
        f"| Incompatible septets skipped | `{incompatible_septs}` |",
    ]
    if best_single is not None:
        lines.append(f"| Best single delta | `{best_single['single_delta_vs_current_bits']:.1f}` |")
    if best_sept is not None:
        lines.extend(
            [
                f"| Best septet total bits | `{best_sept['total_bits']:.1f}` |",
                f"| Best septet delta vs current | `{best_sept['delta_vs_current_bits']:.1f}` |",
            ]
        )
    lines.extend(
        [
    ...
    for row in (best_sept or {"repairs": []})["repairs"]:
        lines.append(
    ...
    write_result("54_post_forced_repair_sept_search", result, lines)
