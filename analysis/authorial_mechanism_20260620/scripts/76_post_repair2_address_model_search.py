...
def build_rows(formula: dict, books: dict[str, str], scorer) -> tuple[list[dict], list[dict], list[dict]]:
    ...
    for book_index, book in enumerate(order):
        ...
        for op_index, op in enumerate(formula["book_recipes"][book]["ops"]):
            ...
            if op["type"] == "literal":
                ...
                remaining = sum(int(row["length"]) for row in formula["book_recipes"][book]["ops"][op_index:])
                max_length = min(remaining, emitted_len - source_digit_pos)
                symbol_count = max_length - min_len + 1
                length_index = length - min_len
                length_code_bits = scorer.truncated_binary_bits(symbol_count, length_index)
                ...
                copy_rows.append(
                    {
                        ...
                        "length": length,
                        "chunk": chunk,
                        "length_code_bits": length_code_bits,
...
def main() -> None:
    ...
    _spans, literal_rows, copy_rows = build_rows(formula, books, scorer)
    ...
    if (
        ...
        classification = "post_repair2_address_model_candidate"
    elif best_any["total_bits"] < current_bits - 1e-9 and not best_any["decodable"]:
        classification = "post_repair2_address_optimistic_only_not_promoted"
    else:
        classification = "post_repair2_address_absolute_minaddr_retained"

    result = {
        "schema": "post_repair2_address_model_search.v1",
        "test": "76_post_repair2_address_model_search",
        ...
        "promotion_rule": (
            ...
            "min_len-bounded absolute digit source positions under full post-repair2 rescoring"
        ),
    ...
    write_result("76_post_repair2_address_model_search", result, lines)
