...
def build_rows(formula: dict, books: dict[str, str], score: dict) -> tuple[list[dict], list[dict], list[dict]]:
    ...
    copy_id = 0
    literal_id = 0
    length_rows = score["midpoint_copy_length_rows"]
                ...
                copy_rows.append(
                    {
                        ...
                        "length": length,
                        "chunk": chunk,
                        "length_code_bits": length_by_copy_id[copy_id]["adaptive_context_bits"],
...
def optimize_sparse_seed_runs(seed_possible: list[bool], seed_savings: list[float]) -> dict:
    intervals = []
    for start in range(len(seed_possible)):
        if not seed_possible[start]:
            continue
        savings = 0.0
        for end in range(start, len(seed_possible)):
...
def main() -> None:
    ...
    _spans, literal_rows, copy_rows = build_rows(formula, books, current_score)
    ...
    if (
        ...
        classification = "post_midpoint_alpha1_address_model_candidate"
    elif best_any["total_bits"] < current_bits - 1e-9 and not best_any["decodable"]:
        classification = "post_midpoint_alpha1_address_optimistic_only_not_promoted"
    else:
        classification = "post_midpoint_alpha1_address_absolute_minaddr_retained"

    result = {
        "schema": "post_midpoint_alpha1_address_model_search.v1",
        "test": "89_post_midpoint_alpha1_address_model_search",
        ...
        "promotion_rule": (
            ...
            "min_len-bounded absolute digit source positions under full midpoint alpha=1 rescoring"
        ),
    ...
    write_result("89_post_midpoint_alpha1_address_model_search", result, lines)
