...
OUT_FORMULA = HERE / "sequential_lz_digit_address_contextual_bounded_adaptive_copy_length_midpoint_minaddr_repair3_formula_469.json"
...
OUT_TOTAL_KEY = "sequential_lz_digit_address_contextual_bounded_adaptive_copy_length_midpoint_minaddr_repair3_bits"
...
def midpoint_copy_length_bits(formula: dict, books: dict[str, str], context_module) -> tuple[float, list[dict], dict[str, int]]:
    rows = context_module.collect_copy_rows(formula, books)
    alpha = int(formula["policy"]["copy_length_model"]["alpha"])
    return context_module.adaptive_context_bits(rows, alpha, midpoint_context)


def score_formula(formula: dict, books: dict[str, str], frontier, context_module) -> dict:
    base = frontier.score_formula(formula, books)
    midpoint_bits, midpoint_rows, context_counts = midpoint_copy_length_bits(formula, books, context_module)
    current_copy_length = float(base["copy_length_code_bits"])
    total_bits = float(base["total_bits"]) - current_copy_length + midpoint_bits
    copy_bits = float(base["copy_address_bits"]) + midpoint_bits
    return {
        **base,
        "total_bits": total_bits,
        "copy_bits": copy_bits,
        "copy_length_code_bits": midpoint_bits,
        "midpoint_copy_length_rows": midpoint_rows,
        "midpoint_copy_length_context_counts": context_counts,
        "truncated_copy_length_bits": current_copy_length,
    }


def find_best_repair(formula: dict, books: dict[str, str], current_bits: float, frontier, context_module) -> tuple[dict | None, dict]:
...
def main() -> None:
    ...
    current_score = score_formula(formula, books, frontier, context_module)
    ...
    best, counts = find_best_repair(formula, books, current_bits, frontier, context_module)
    promoted = best is not None and best["delta_bits"] < -1e-9
    classification = (
        "controlled_post_midpoint_local_repair_improvement"
        if promoted
        else "post_midpoint_local_frontier_closed"
    )
    ...
    if promoted:
        ...
        out.update(
            {
                "schema": "sequential_lz_digit_address_contextual_bounded_adaptive_copy_length_midpoint_minaddr_repair3_formula.v1",
                ...
                "mdl_estimate_rough": {
                    **out["mdl_estimate_rough"],
                    ...
                    "gain_vs_previous_midpoint_bits": current_bits - score["total_bits"],
                ...
                "post_midpoint_local_repair": strip_score(best),
                "validation": {
                    **out["validation"],
                    "post_midpoint_local_repair_roundtrip_audit": score["validation"],
    ...
    result = {
        "schema": "post_midpoint_local_frontier.v1",
        "test": "85_post_midpoint_local_frontier",
        "classification": classification,
        "translation_delta": "NONE",
        ...
        "promotion_rule": (
            ...
            "beats the active midpoint-context adaptive copy-length formula under "
            "full rescoring"
        ),
    ...
    write_result("85_post_midpoint_local_frontier", result, lines)
