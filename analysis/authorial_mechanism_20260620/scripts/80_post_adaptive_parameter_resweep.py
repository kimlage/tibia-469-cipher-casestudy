...
FRONTIER = HERE / "scripts/79_post_adaptive_copy_length_local_frontier.py"
...
def load_frontier_module():
    spec = importlib.util.spec_from_file_location("post_adaptive_frontier", FRONTIER)
...
def copy_length_declaration_bits(alpha: int, inherited_bounded_bits: int) -> int:
    return inherited_bounded_bits + gamma_bits(alpha + 1)
...
def main() -> None:
    ...
    current_score = frontier.score_formula(formula, books, frontier.load_frontier_module())
    ...
    inherited_copy_bounded_decl = int(
        formula["policy"]["copy_length_model"]["replaces"]["model_declaration_bits"]
    )
    ...
    for literal_k in range(0, 11):
        ...
        score = frontier.score_formula(candidate, books, frontier.load_frontier_module())
        rows.append(
            {
    ...
    for alpha in range(1, 65):
        declaration_bits = copy_length_declaration_bits(alpha, inherited_copy_bounded_decl)
        candidate = copy.deepcopy(formula)
        ...
        rows.append(
            {
                "family": "adaptive_copy_length_alpha",
                "parameter": alpha,
                "total_bits": score["total_bits"],
                "delta_vs_current_bits": score["total_bits"] - current_bits,
    ...
    best = rows[0]
    classification = (
        "post_adaptive_parameter_resweep_candidate"
        if best["total_bits"] < current_bits - 1e-9
        else "post_adaptive_parameter_resweep_retains_current"
    )

    result = {
        "schema": "post_adaptive_parameter_resweep.v1",
        "test": "80_post_adaptive_parameter_resweep",
        ...
        "promotion_rule": (
            ...
            "item-type context, or adaptive copy-length alpha parameter beats the "
            "active post-adaptive formula"
        ),
    ...
    write_result("80_post_adaptive_parameter_resweep", result, lines)
