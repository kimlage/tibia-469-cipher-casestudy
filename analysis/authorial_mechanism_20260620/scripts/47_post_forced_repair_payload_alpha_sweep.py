...
def main() -> None:
    ...
    current_bits = formula["mdl_estimate_rough"][
    ...
    current_payload_plus_model = adaptive_payload_bits(stream, current_alpha) + gamma_bits(current_alpha + 1)
        ...
        rows.append(
            {
                ...
                "delta_vs_current_alpha_bits": payload_bits + declaration_bits - current_payload_plus_model,
            }
        )
    rows.sort(key=lambda row: row["payload_plus_model_bits"])
    best = rows[0]
    classification = (
        "post_forced_repair_payload_alpha_retains_14"
        if best["alpha"] == current_alpha
        else "post_forced_repair_payload_alpha_candidate"
    )

    result = {
        "schema": "post_forced_repair_payload_alpha_sweep.v1",
        "test": "47_post_forced_repair_payload_alpha_sweep",
    ...
    write_result("47_post_forced_repair_payload_alpha_sweep", result, lines)
