                ...
                rows.append(
                    {
                        ...
                        "remaining": remaining,
                        "source_first_length_symbol_count": min(remaining, emitted_len - source_digit_pos) - min_len + 1,
                        "length_first_length_symbol_count": min(remaining, emitted_len) - min_len + 1,
...
def adaptive_length_bits(rows: list[dict], alpha: int, symbol_count_key: str) -> tuple[float, list[float]]:
    counts: dict[int, int] = {}
    total = 0.0
    bits_by_copy = []
    for row in rows:
...
def main() -> None:
    ...
    source_length_bits, source_length_by_copy = adaptive_length_bits(
        copy_rows, alpha, "source_first_length_symbol_count"
    )
    length_first_bits, length_first_by_copy = adaptive_length_bits(
        copy_rows, alpha, "length_first_length_symbol_count"
    )
    ...
    best_any = rows[0]
    if best_decodable["model"] != "source_first_then_adaptive_length_active" and best_decodable["total_bits"] < current_bits - 1e-9:
        classification = "post_adaptive_copy_order_candidate"
    elif best_any["total_bits"] < current_bits - 1e-9 and not best_any["decodable"]:
        classification = "post_adaptive_copy_order_optimistic_only_not_promoted"
    else:
        classification = "post_adaptive_copy_order_source_first_retained"
    ...
    result = {
        "schema": "post_adaptive_copy_order_search.v1",
        "test": "83_post_adaptive_copy_order_search",
        ...
        "promotion_rule": (
            ...
            "then adaptive length under full post-adaptive rescoring"
        ),
    ...
    write_result("83_post_adaptive_copy_order_search", result, lines)
