...
OUT_FORMULA = HERE / "sequential_lz_digit_address_contextual_bounded_adaptive_copy_length_context2_alpha1_minaddr_repair2_formula_469.json"
...
OUT_TOTAL_KEY = "sequential_lz_digit_address_contextual_bounded_adaptive_copy_length_context2_alpha1_minaddr_repair2_bits"
...
def context_declaration_bits(base_declaration_bits: int, alpha: int, context_count: int) -> int:
    return base_declaration_bits + gamma_bits(alpha + 1) + 1 + gamma_bits(context_count + 1)


def searched_split_declaration_bits(
    base_declaration_bits: int,
    alpha: int,
    context_count: int,
    split_book: int,
) -> int:
    return context_declaration_bits(base_declaration_bits, alpha, context_count) + gamma_bits(split_book + 1)
...
def model_row(
    *,
    ...
    extra_declaration_bits: int = 0,
    decodable: bool = True,
) -> dict:
    length_bits, audit_rows, context_counts = context_search.adaptive_context_bits(rows, alpha, context_fn)
    context_count = len(context_counts)
    declaration_bits = (
        context_declaration_bits(copy_base_declaration_bits, alpha, context_count)
        + extra_declaration_bits
    )
...
def main() -> None:
    ...
    active_model = next(row for row in models if row["model"] == "active_book_midpoint_35_context")
    if abs(active_model["total_bits"] - current_bits) > 1e-6:
        raise RuntimeError((active_model["total_bits"], current_bits))
    ...
    classification = (
        "controlled_post_midpoint_alpha1_copy_length_context_improvement"
        if promoted
        else "post_midpoint_alpha1_copy_length_context_retains_midpoint"
    )

    if promoted:
        out = copy.deepcopy(formula)
        out["schema"] = "sequential_lz_digit_address_contextual_bounded_adaptive_copy_length_context2_alpha1_minaddr_repair2_formula.v1"
        ...
        out["validation"] = {
            **out["validation"],
            "post_midpoint_alpha1_copy_length_context_resweep_roundtrip_audit": current_score["validation"],
            "post_midpoint_alpha1_copy_length_context_copy_items": len(rows),
        }
        OUT_FORMULA.write_text(json.dumps(out, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    result = {
        "schema": "post_midpoint_alpha1_copy_length_context_resweep.v1",
        "test": "91_post_midpoint_alpha1_copy_length_context_resweep",
    ...
    write_result("91_post_midpoint_alpha1_copy_length_context_resweep", result, lines)
