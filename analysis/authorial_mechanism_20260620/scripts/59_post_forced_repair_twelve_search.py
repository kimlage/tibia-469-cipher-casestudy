...
def main() -> None:
    ...
    best_twelve = None
    compatible_twelves = 0
    incompatible_twelves = 0
    for repairs in itertools.combinations(candidates, 12):
        if not repairs_mutually_compatible(pair_module, repairs):
            incompatible_twelves += 1
            continue
        compatible_twelves += 1
        ...
        if best_twelve is None or row["total_bits"] < best_twelve["total_bits"]:
            best_twelve = row

    best_single = min(candidates, key=lambda row: row["single_total_bits"]) if candidates else None
    promoted = best_twelve is not None and best_twelve["total_bits"] < current_bits
    classification = (
        "post_forced_repair_twelve_improvement"
        if promoted
        else "post_forced_repair_twelve_not_promoted"
    )

    result = {
        "schema": "post_forced_repair_twelve_search.v1",
        "test": "59_post_forced_repair_twelve_search",
        ...
        "compatible_twelves_tested": compatible_twelves,
        "incompatible_twelves_skipped": incompatible_twelves,
        "total_twelves_considered": compatible_twelves + incompatible_twelves,
        "best_single_repair": best_single,
        "best_twelve_repair": best_twelve,
        "boundary": formula["boundary"],
    }

    lines = [
        "# Post-Forced-Repair Twelve-Repair Search",
        "",
        f"Verdict: `{classification}`. Translation delta: `NONE`.",
        "",
        "This audit tests whether twelve compatible literal-to-copy repairs become",
        ...
        f"| Compatible twelve-repair sets tested | `{compatible_twelves}` |",
        f"| Incompatible twelve-repair sets skipped | `{incompatible_twelves}` |",
    ]
    if best_single is not None:
        lines.append(f"| Best single delta | `{best_single['single_delta_vs_current_bits']:.1f}` |")
    if best_twelve is not None:
        lines.extend(
            [
                f"| Best twelve-repair total bits | `{best_twelve['total_bits']:.1f}` |",
                f"| Best twelve-repair delta vs current | `{best_twelve['delta_vs_current_bits']:.1f}` |",
            ]
        )
    lines.extend(
        [
            "",
            "## Best Twelve-Repair Set",
            "",
            "| Book | Op | Book pos | Literal offset | Chunk | Source digit pos | Length | Single delta |",
            "|---:|---:|---:|---:|---|---:|---:|---:|",
        ]
    )
    for row in (best_twelve or {"repairs": []})["repairs"]:
        lines.append(
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            "A twelve-repair set is promoted only if exact rescoring beats the active",
            "forced-length repaired formula. If the best set remains worse, the",
            "local literal-to-copy frontier is closed under compatible twelve-repair",
    ...
    write_result("59_post_forced_repair_twelve_search", result, lines)
