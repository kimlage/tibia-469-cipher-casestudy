    ...
    for book_index, book in enumerate(order):
        ...
        for op in formula["book_recipes"][book]["ops"]:
            if op["type"] == "literal":
                ...
                source_pos_with_separators = int(op.get("source_pos_with_separators_audit", op["source_pos"]))
...
def main() -> None:
    ...
    if best_decodable["model"] != "absolute_digit_source_pos" and best_decodable["total_bits"] < current_bits:
        classification = "digit_address_model_candidate"
    elif best_any["total_bits"] < current_bits and not best_any["decodable"]:
        classification = "digit_address_optimistic_only_not_promoted"
    else:
        classification = "digit_address_absolute_retained"

    result = {
        "schema": "digit_address_model_search.v1",
        "test": "36_digit_address_model_search",
    ...
    write_result("36_digit_address_model_search", result, lines)
