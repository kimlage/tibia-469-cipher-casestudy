...
def main() -> None:
    ...
    for repairs in itertools.combinations(candidates, 3):
    ...
    promoted = best_triple is not None and best_triple["total_bits"] < current_bits
    classification = (
        "post_forced_repair_triple_improvement"
        if promoted
        else "post_forced_repair_triple_not_promoted"
    )

    result = {
        "schema": "post_forced_repair_triple_search.v1",
        "test": "50_post_forced_repair_triple_search",
        ...
        "compatible_triples_tested": compatible_triples,
        "incompatible_triples_skipped": incompatible_triples,
        "total_triples_considered": compatible_triples + incompatible_triples,
        "best_single_repair": best_single,
        "best_triple_repair": best_triple,
    ...
    lines = [
        ...
        f"| Compatible triples tested | `{compatible_triples}` |",
        f"| Incompatible triples skipped | `{incompatible_triples}` |",
    ]
    ...
    if best_triple is not None:
        lines.extend(
            [
                f"| Best triple total bits | `{best_triple['total_bits']:.1f}` |",
                f"| Best triple delta vs current | `{best_triple['delta_vs_current_bits']:.1f}` |",
            ]
        )
    lines.extend(
        [
    ...
    for row in (best_triple or {"repairs": []})["repairs"]:
        lines.append(
    ...
    write_result("50_post_forced_repair_triple_search", result, lines)
