    ...
    for book_index, book in enumerate(order):
        ...
        for op in formula["book_recipes"][book]["ops"]:
            if op["type"] == "literal":
                ...
                if len(text) != length:
                    raise RuntimeError((book, "literal_length_mismatch", op))
                literal_rows.append(
                    {
                ...
                if len(chunk) != length:
                    raise RuntimeError((book, "short_copy", op))
                copy_rows.append(
                    {
                        "copy_id": copy_id,
                        "target_book": book,
...
def absolute_copy_bits(copy_rows: list[dict]) -> float:
    return sum(
        math.log2(max(2, row["target_digit_global"])) + row["length_code_bits"]
        for row in copy_rows
    )
...
def main() -> None:
    ...
    current_bits = formula["mdl_estimate_rough"]["sequential_lz_digit_address_forced_length_literal_repair_bits"]
    ...
    if best_decodable["model"] != "absolute_digit_source_pos" and best_decodable["total_bits"] < current_bits:
        classification = "post_forced_repair_address_model_candidate"
    elif best_any["total_bits"] < current_bits and not best_any["decodable"]:
        classification = "post_forced_repair_address_optimistic_only_not_promoted"
    else:
        classification = "post_forced_repair_address_absolute_retained"

    result = {
        "schema": "post_forced_repair_address_model_search.v1",
        "test": "48_post_forced_repair_address_model_search",
    ...
    write_result("48_post_forced_repair_address_model_search", result, lines)
