...
def main() -> None:
    ...
    best_quad = None
    compatible_quads = 0
    incompatible_quads = 0
    for repairs in itertools.combinations(candidates, 4):
        if not repairs_mutually_compatible(pair_module, repairs):
            incompatible_quads += 1
            continue
        compatible_quads += 1
        ...
        if best_quad is None or row["total_bits"] < best_quad["total_bits"]:
            best_quad = row

    best_single = min(candidates, key=lambda row: row["single_total_bits"]) if candidates else None
    promoted = best_quad is not None and best_quad["total_bits"] < current_bits
    classification = (
        "post_forced_repair_quad_improvement"
        if promoted
        else "post_forced_repair_quad_not_promoted"
    )

    result = {
        "schema": "post_forced_repair_quad_search.v1",
        "test": "51_post_forced_repair_quad_search",
        ...
        "compatible_quads_tested": compatible_quads,
        "incompatible_quads_skipped": incompatible_quads,
        "total_quads_considered": compatible_quads + incompatible_quads,
        "best_single_repair": best_single,
        "best_quad_repair": best_quad,
    ...
    lines = [
        ...
        f"| Compatible quads tested | `{compatible_quads}` |",
        f"| Incompatible quads skipped | `{incompatible_quads}` |",
    ]
    ...
    if best_quad is not None:
        lines.extend(
            [
                f"| Best quad total bits | `{best_quad['total_bits']:.1f}` |",
                f"| Best quad delta vs current | `{best_quad['delta_vs_current_bits']:.1f}` |",
            ]
        )
    lines.extend(
        [
    ...
    for row in (best_quad or {"repairs": []})["repairs"]:
        lines.append(
    ...
    write_result("51_post_forced_repair_quad_search", result, lines)
