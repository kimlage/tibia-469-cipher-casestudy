...
def main() -> None:
    ...
    best_quint = None
    compatible_quints = 0
    incompatible_quints = 0
    for repairs in itertools.combinations(candidates, 5):
        if not repairs_mutually_compatible(pair_module, repairs):
            incompatible_quints += 1
            continue
        compatible_quints += 1
        ...
        if best_quint is None or row["total_bits"] < best_quint["total_bits"]:
            best_quint = row

    best_single = min(candidates, key=lambda row: row["single_total_bits"]) if candidates else None
    promoted = best_quint is not None and best_quint["total_bits"] < current_bits
    classification = (
        "post_forced_repair_quint_improvement"
        if promoted
        else "post_forced_repair_quint_not_promoted"
    )

    result = {
        "schema": "post_forced_repair_quint_search.v1",
        "test": "52_post_forced_repair_quint_search",
        ...
        "compatible_quints_tested": compatible_quints,
        "incompatible_quints_skipped": incompatible_quints,
        "total_quints_considered": compatible_quints + incompatible_quints,
        "best_single_repair": best_single,
        "best_quint_repair": best_quint,
    ...
    lines = [
        ...
        f"| Compatible quints tested | `{compatible_quints}` |",
        f"| Incompatible quints skipped | `{incompatible_quints}` |",
    ]
    if best_single is not None:
        lines.append(f"| Best single delta | `{best_single['single_delta_vs_current_bits']:.1f}` |")
    if best_quint is not None:
        lines.extend(
            [
                f"| Best quint total bits | `{best_quint['total_bits']:.1f}` |",
                f"| Best quint delta vs current | `{best_quint['delta_vs_current_bits']:.1f}` |",
            ]
        )
    lines.extend(
        [
    ...
    for row in (best_quint or {"repairs": []})["repairs"]:
        lines.append(
    ...
    write_result("52_post_forced_repair_quint_search", result, lines)
