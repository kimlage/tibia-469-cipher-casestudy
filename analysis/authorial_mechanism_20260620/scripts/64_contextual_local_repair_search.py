...
OUT_FORMULA = HERE / "sequential_lz_digit_address_contextual_repair_formula_469.json"
BOOKS_DIGITS = ROOT / "analysis/audit_20260609/books_digits.json"

OUT_TOTAL_KEY = "sequential_lz_digit_address_contextual_repair_bits"
    ...
    for book in map(str, formula["policy"]["book_order"]):
        ...
        for op_index, op in enumerate(formula["book_recipes"][book]["ops"]):
            ...
            if op["type"] == "literal":
                ...
                chunk = emitted[int(op["source_digit_pos"]) : int(op["source_digit_pos"]) + length]
                emitted += chunk
            else:
                raise ValueError(op)
            book_pos += length
...
def main() -> None:
    ...
    best, tested = find_best_single_repair(formula, books, current_bits)
    promoted = best is not None and best["delta_bits"] < -1e-9
    classification = (
        "controlled_contextual_local_repair_improvement"
        if promoted
        else "contextual_local_repair_not_promoted"
    )
    ...
    if promoted:
        ...
        out.update(
            {
                "schema": "sequential_lz_digit_address_contextual_repair_formula.v1",
                ...
                "validation": {
                    **out["validation"],
                    "contextual_repair_roundtrip_audit": score["validation"],
    ...
    result = {
        "schema": "contextual_local_repair_search.v1",
        "test": "64_contextual_local_repair_search",
        "classification": classification,
        "translation_delta": "NONE",
        ...
        "tested_repairs": tested,
        "best_repair": best,
        "promotion_rule": (
            "promote only if a single valid literal-to-copy replacement beats the active "
            "contextual formula under exact rescoring with payload context order and item-type context order"
        ),
    ...
    lines = [
        ...
        f"- Candidate repairs tested: `{tested}`",
    ]
    if best is None:
        ...
        lines.extend(
            [
                ...
                f"- Best repair: book `{best['book']}`, text `{best['text']}`,",
                f"  literal offset `{best['literal_offset']}`, length `{best['length']}`,",
    ...
    write_result("64_contextual_local_repair_search", result, lines)
