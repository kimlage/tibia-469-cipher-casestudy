...
def main() -> None:
    ...
    best_sext = None
    compatible_sexts = 0
    incompatible_sexts = 0
    for repairs in itertools.combinations(candidates, 6):
        if not repairs_mutually_compatible(pair_module, repairs):
            incompatible_sexts += 1
            continue
        compatible_sexts += 1
        ...
        if best_sext is None or row["total_bits"] < best_sext["total_bits"]:
            best_sext = row

    best_single = min(candidates, key=lambda row: row["single_total_bits"]) if candidates else None
    promoted = best_sext is not None and best_sext["total_bits"] < current_bits
    classification = (
        "post_forced_repair_sext_improvement"
        if promoted
        else "post_forced_repair_sext_not_promoted"
    )

    result = {
        "schema": "post_forced_repair_sext_search.v1",
        "test": "53_post_forced_repair_sext_search",
        ...
        "compatible_sexts_tested": compatible_sexts,
        "incompatible_sexts_skipped": incompatible_sexts,
        "total_sexts_considered": compatible_sexts + incompatible_sexts,
        "best_single_repair": best_single,
        "best_sext_repair": best_sext,
    ...
    lines = [
        ...
        f"| Compatible sextets tested | `{compatible_sexts}` |",
        f"| Incompatible sextets skipped | `{incompatible_sexts}` |",
    ]
    if best_single is not None:
        lines.append(f"| Best single delta | `{best_single['single_delta_vs_current_bits']:.1f}` |")
    if best_sext is not None:
        lines.extend(
            [
                f"| Best sextet total bits | `{best_sext['total_bits']:.1f}` |",
                f"| Best sextet delta vs current | `{best_sext['delta_vs_current_bits']:.1f}` |",
            ]
        )
    lines.extend(
        [
    ...
    for row in (best_sext or {"repairs": []})["repairs"]:
        lines.append(
    ...
    write_result("53_post_forced_repair_sext_search", result, lines)
