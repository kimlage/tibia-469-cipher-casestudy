...
See [15_leave_one_book_out_book_bounded_source_audit.md](test_results/15_leave_one_book_out_book_bounded_source_audit.md).
...
See [16_leave_one_book_out_family_excluded_source_audit.md](test_results/16_leave_one_book_out_family_excluded_source_audit.md).
...
See [19_seeded_online_formula_rescore_audit.md](test_results/19_seeded_online_formula_rescore_audit.md).
...
formula scoring.
See [20_seeded_rescore_loss_decomposition.md](test_results/20_seeded_rescore_loss_decomposition.md).
...
See [35_copy_source_state_compression_gate.md](test_results/35_copy_source_state_compression_gate.md).
...
See [42_full_corpus_source_substitution_frontier_gate.md](test_results/42_full_corpus_source_substitution_frontier_gate.md).
...
See [43_full_corpus_source_substitution_second_pass_gate.md](test_results/43_full_corpus_source_substitution_second_pass_gate.md).
...
See [46_source_substitution_saturation_audit.md](test_results/46_source_substitution_saturation_audit.md).
...
See [31_source_selection_derivation_boundary_gate.md](test_results/31_source_selection_derivation_boundary_gate.md).
...
See [51_copy_length_segmentation_exception_audit.md](test_results/51_copy_length_segmentation_exception_audit.md).
...
resegmentation.
See [52_targetmax_resegmentation_candidate_audit.md](test_results/52_targetmax_resegmentation_candidate_audit.md).

### Target-Max Resegmentation Formula Gate

The exact component scorer then checks the best proxy candidate against the current source-substitution ledger. It reproduces the current bound at `8160.825608` bits, then promotes the book `9` op `0` `preserve_next_mode` resegmentation with slack `4`.
The candidate lowers the bound from `8160.825608` to `8158.766094` bits, a gain of `+2.059513` bits. The improvement is almost entirely copy-length cost (`-2.062867` bits), with a small copy-source penalty (`+0.003353` bits), and zero literal-payload/item-type change.
Roundtrip and score errors are both `0`/`0`. This is a real compression-bound promotion, not row0 derivation or semantics.
See [53_targetmax_resegmentation_formula_gate.md](test_results/53_targetmax_resegmentation_formula_gate.md).

### Target-Max Resegmentation Second-Pass Gate

The next exact gate retests the remaining compatible target-max rewrites against the promoted formula. It skips `1` stale exception already changed by gate 53, tests `44` candidates, finds `40` valid candidates and `4` exact improvements.
The best candidate is book `2` op `9` `preserve_next_mode` with slack `1`, lowering the bound from `8158.766094` to `8157.065654` bits, a gain of `+1.700440` bits.
...
See [54_targetmax_resegmentation_second_pass_gate.md](test_results/54_targetmax_resegmentation_second_pass_gate.md).
...
See [55_targetmax_resegmentation_saturation_gate.md](test_results/55_targetmax_resegmentation_saturation_gate.md).

### Post-Target-Max Source Substitution Frontier Gate

After target-max saturation, the exact same-chunk source frontier is rerun because the resegmented source stream changed. It finds a microscopic positive pair: active `8156.050355` to candidate `8156.050167` bits, gain `+0.000188`.
...
The follow-up stop audit freezes this micro-frontier as non-mainline: the two post-target-max source passes sum to only `0.000369` bits while the minimum selector floor total is larger by `32.244` bits.
...
See [58_post_targetmax_source_substitution_stop_audit.md](test_results/58_post_targetmax_source_substitution_stop_audit.md).
...
See [59_active_formula_dependency_refresh_gate.md](test_results/59_active_formula_dependency_refresh_gate.md).

### Active Source-Length Joint Refresh Gate

The source/length refresh repeats the gate-49 joint tests on the active post-target-max formula. Encoder target-max length hits improve by `+4`, but decoder-valid joint rules do not improve.
...
See [60_active_source_length_joint_refresh_gate.md](test_results/60_active_source_length_joint_refresh_gate.md).
...
See [61_active_copy_length_exception_topology_gate.md](test_results/61_active_copy_length_exception_topology_gate.md).

### Active Residual Target-Max Resegmentation Gate

The residual target-max gate tests the remaining active exceptions with the same local extend-and-trim modes used by the promoted target-max gates. It tests `38` candidates, with `34` valid and `0` improving.
...
See [67_partial_boundary_shift_second_pass_gate.md](test_results/67_partial_boundary_shift_second_pass_gate.md).
...
See [68_partial_boundary_shift_second_pass_formula_gate.md](test_results/68_partial_boundary_shift_second_pass_formula_gate.md).
...
See [69_partial_boundary_shift_saturation_gate.md](test_results/69_partial_boundary_shift_saturation_gate.md).

### Recent Formula Row0 Compatibility Audit

A compatibility gate then asks whether those recent book-formula promotions change the independent row0 conclusion. They do not. The current bound is `8154.676268` bits, but the gates contain no row0 label holdout predictor, do not beat row0 lookup after paid anchor/rule cost, do not derive `39`, `93`, or `19/91`, and add no CipSoft/authorial provenance.
Result: `book_generation_improved_row0_unchanged`; `row0 unchanged`.
See [70_recent_formula_row0_compatibility_audit.md](test_results/70_recent_formula_row0_compatibility_audit.md).
...
See [71_final_formula_dependency_refresh_gate.md](test_results/71_final_formula_dependency_refresh_gate.md).
...
See [47_row0_parallel_provenance_bridge_audit.md](test_results/47_row0_parallel_provenance_bridge_audit.md).
