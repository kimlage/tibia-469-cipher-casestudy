...
POST_TARGETMAX_SOURCE_SUBSTITUTION_GATE = (
    ROOT
    / "analysis"
    / "prequential_and_row0_origin_audit_20260621"
    / "reports"
    / "test_results"
    / "56_post_targetmax_source_substitution_frontier_gate.json"
)
...
def make_result(legacy: dict[str, Any]) -> dict[str, Any]:
    ...
    targetmax_resegmentation_formula = load_json(TARGETMAX_RESEGMENTATION_FORMULA_GATE)
    assert_analysis_boundary(
        "targetmax_resegmentation_formula_gate",
    ...
    post_targetmax_source_substitution = load_json(
        POST_TARGETMAX_SOURCE_SUBSTITUTION_GATE
    )
    assert_analysis_boundary(
        "post_targetmax_source_substitution_frontier_gate",
        post_targetmax_source_substitution,
    )
    post_targetmax_source_substitution_second_pass = load_json(
        POST_TARGETMAX_SOURCE_SUBSTITUTION_SECOND_PASS_GATE
    )
    assert_analysis_boundary(
        "post_targetmax_source_substitution_second_pass_gate",
    ...
    return {
        ...
        "targetmax_resegmentation_formula": {
            "classification": targetmax_resegmentation_formula["classification"],
            "source": rel(TARGETMAX_RESEGMENTATION_FORMULA_GATE),
            "scope": targetmax_resegmentation_formula["scope"],
            "summary": targetmax_resegmentation_formula["summary"],
            "decision": targetmax_resegmentation_formula["decision"],
            "candidate_output_formula": targetmax_resegmentation_formula[
                "candidate_output_formula"
            ],
        },
        "targetmax_resegmentation_second_pass": {
        ...
        "post_targetmax_source_substitution_frontier": {
            "classification": post_targetmax_source_substitution["classification"],
            "source": rel(POST_TARGETMAX_SOURCE_SUBSTITUTION_GATE),
            "scope": post_targetmax_source_substitution["scope"],
            "summary": post_targetmax_source_substitution["summary"],
            "decision": post_targetmax_source_substitution["decision"],
            "candidate_output_formula": post_targetmax_source_substitution[
                "candidate_output_formula"
            ],
        },
        "post_targetmax_source_substitution_second_pass": {
            "classification": post_targetmax_source_substitution_second_pass[
                "classification"
            ],
            "source": rel(POST_TARGETMAX_SOURCE_SUBSTITUTION_SECOND_PASS_GATE),
            "scope": post_targetmax_source_substitution_second_pass["scope"],
            "summary": post_targetmax_source_substitution_second_pass["summary"],
            "decision": post_targetmax_source_substitution_second_pass["decision"],
            "candidate_output_formula": post_targetmax_source_substitution_second_pass[
                "candidate_output_formula"
            ],
        },
        "post_targetmax_source_substitution_stop": {
...
def render_markdown(
    ...
    *,
    ...
    post_targetmax_source = result["post_targetmax_source_substitution_frontier"][
        "summary"
    ]
    ...
    lines.extend(
        [
            ...
            "",
            "The exact component scorer then checks the best proxy candidate against "
            "the current source-substitution ledger. It reproduces the current bound "
            f"at `{targetmax_formula['current_exact_total_bits']:.6f}` bits, then "
            f"promotes the book `{targetmax_formula['book']}` op "
            f"`{targetmax_formula['op_index']}` `{targetmax_formula['mode']}` "
            f"resegmentation with slack `{targetmax_formula['slack']}`.",
            "The candidate lowers the bound from "
            f"`{targetmax_formula['current_total_bits']:.6f}` to "
            f"`{targetmax_formula['candidate_total_bits']:.6f}` bits, a gain of "
            f"`{targetmax_formula['candidate_gain_bits']:+.6f}` bits. The "
            "improvement is almost entirely copy-length cost "
            f"(`{targetmax_formula['component_delta_bits']['copy_length_bits']:+.6f}` "
            "bits), with a small copy-source penalty "
            f"(`{targetmax_formula['component_delta_bits']['copy_source_bits']:+.6f}` "
            "bits), and zero literal-payload/item-type change.",
            "Roundtrip and score errors are both "
            f"`{targetmax_formula['roundtrip_error_count']}`/"
            f"`{targetmax_formula['score_error_count']}`. This is a real "
            ...
            "",
            "After target-max saturation, the exact same-chunk source frontier is "
            "rerun because the resegmented source stream changed. It finds a "
            "microscopic positive pair: active "
            f"`{post_targetmax_source['active_total_bits']:.6f}` to candidate "
            f"`{post_targetmax_source['candidate_total_bits']:.6f}` bits, gain "
            f"`{post_targetmax_source['candidate_gain_bits']:+.6f}`.",
            "The search covers "
            f"`{post_targetmax_source['single_substitution_count']}` singles and "
            f"`{post_targetmax_source['pair_substitution_count']}` pairs, with "
            f"`{post_targetmax_source['positive_single_count']}` positive singles "
            f"and `{post_targetmax_source['positive_pair_count']}` positive pairs. "
            "This updates the compression bound only; segmentation, copy lengths, "
            "`row0`, and semantics are unchanged.",
            f"See [56_post_targetmax_source_substitution_frontier_gate.md]({post_targetmax_source_substitution_frontier_gate_link}).",
            "",
            "### Post-Target-Max Source Substitution Second-Pass Gate",
            "",
            "A second pass over the same post-target-max source frontier still finds "
            "another microscopic pair: active "
            f"`{post_targetmax_source_second['active_total_bits']:.6f}` to "
            f"candidate `{post_targetmax_source_second['candidate_total_bits']:.6f}` "
            f"bits, gain `{post_targetmax_source_second['candidate_gain_bits']:+.6f}`.",
            "The search covers "
            f"`{post_targetmax_source_second['single_substitution_count']}` singles "
            f"and `{post_targetmax_source_second['pair_substitution_count']}` pairs. "
            "This is still fixed-recipe compression bookkeeping, not stronger "
            "generation evidence.",
...
def main() -> None:
    ...
    report_path.write_text(
        render_markdown(
            ...
            post_targetmax_source_substitution_frontier_gate_link=(
                "56_post_targetmax_source_substitution_frontier_gate.md"
            ),
    ...
    final_report_path.write_text(
        render_markdown(
            ...
            post_targetmax_source_substitution_frontier_gate_link=(
                "test_results/56_post_targetmax_source_substitution_frontier_gate.md"
            ),
            post_targetmax_source_substitution_second_pass_gate_link=(
