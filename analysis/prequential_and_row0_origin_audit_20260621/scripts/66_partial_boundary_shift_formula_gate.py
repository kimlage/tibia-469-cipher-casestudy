...
GATE65 = TEST_RESULTS / "65_active_exception_partial_boundary_shift_gate.json"
    ...
    *,
    ...
    return {
        **formula,
        ...
        "partial_boundary_shift_compile": {
            "source": "66_partial_boundary_shift_formula_gate",
            "candidate_gate": rel(GATE65),
            "book": best["book"],
        ...
        "policy": {
            **formula["policy"],
            "partial_boundary_shift": {
                "source": "66_partial_boundary_shift_formula_gate",
                "scope": "single exact-scored partial shift inside an active residual target-max two-op window",
        ...
        "mdl_estimate_rough": {
            **formula["mdl_estimate_rough"],
            ...
            "gain_vs_previous_targetmax_saturated_source_substitution_second_pass_bits": gain,
...
def make_result() -> dict[str, Any]:
    ...
    assert_boundary("active_exception_partial_boundary_shift_gate", gate65)
    if gate65["classification"] != "active_exception_partial_boundary_shift_candidate_found":
        raise RuntimeError("gate65 did not find a candidate")
    ...
    best = gate65["summary"]["best_valid_candidate"]
    exception = next(
        row
        for row in gate65["candidates"]
    ...
    classification = (
        "partial_boundary_shift_formula_improves_bound"
        if not candidate_errors and candidate_gain > 0
        else "partial_boundary_shift_formula_not_promoted"
    )
    output_formula = None
    if classification == "partial_boundary_shift_formula_improves_bound":
        ...
        output["validation"] = {
            **candidate_formula["validation"],
            "partial_boundary_shift_roundtrip_audit": {
                "book_count": 70,
    ...
    return {
        "schema": "partial_boundary_shift_formula_gate.v1",
        ...
        "scope": {
            ...
            "candidate_source": "best exact-scored partial boundary shift from gate 65",
        },
        "summary": {
        ...
        "decision": {
            "compression_bound_status": (
                "promoted_8155_261037"
                if classification == "partial_boundary_shift_formula_improves_bound"
                else "unchanged_8156_049986"
            ),
...
def main() -> None:
    ...
    (TEST_RESULTS / "66_partial_boundary_shift_formula_gate.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    (TEST_RESULTS / "66_partial_boundary_shift_formula_gate.md").write_text(
