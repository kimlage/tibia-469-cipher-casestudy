...
GATE56_RESULT = TEST_RESULTS / "56_post_targetmax_source_substitution_frontier_gate.json"
...
def patch_output_formula_metadata(result: dict[str, Any]) -> None:
    ...
    compile_info = formula.setdefault(
        "post_targetmax_source_substitution_second_pass_compile", {}
    )
    compile_info.update(
        {
            "source": "57_post_targetmax_source_substitution_second_pass_gate",
            "previous_gate": rel(GATE56_RESULT),
    ...
    formula["policy"] = {
        **formula["policy"],
        "post_targetmax_source_substitution_second_pass": {
            "source": "57_post_targetmax_source_substitution_second_pass_gate",
            "scope": "second exact single and pair same-chunk legal source substitution pass after target-max saturation; segmentation and copy lengths fixed",
    ...
    formula["mdl_estimate_rough"][
        "gain_vs_previous_post_targetmax_source_substitution_bits"
...
def make_result() -> dict[str, Any]:
    gate56 = load_json(GATE56_RESULT)
    assert_boundary("post_targetmax_source_substitution_frontier_gate", gate56)
    ...
    gate42.GATE41_RESULT = GATE56_RESULT
    gate42.ACTIVE_TOTAL_KEY = ACTIVE_TOTAL_KEY
    gate42.OUT_TOTAL_KEY = OUT_TOTAL_KEY

    result = gate42.make_result()
    result["schema"] = "post_targetmax_source_substitution_second_pass_gate.v1"
    result["inputs"] = {
        "source_formula": rel(SOURCE_FORMULA),
        "books_digits": rel(gate42.BOOKS_DIGITS),
        "gate56_result": rel(GATE56_RESULT),
    }
    result["scope"]["starts_after_post_targetmax_source_substitution"] = True
    result["scope"]["pass"] = 2
    ...
    result["decision"]["recipe_discovery_status"] = (
        "fixed_recipe_post_targetmax_source_substitution_second_pass_tested"
    )
    if result["summary"]["candidate_gain_bits"] > 0:
        result["classification"] = (
            "post_targetmax_source_substitution_second_pass_improves_bound"
        )
        result["decision"][
            "compression_bound_status"
        ] = "improved_by_post_targetmax_source_substitution_second_pass"
    else:
        result["classification"] = (
            "post_targetmax_source_substitution_second_pass_frontier_closed"
        )
        result["decision"][
            "compression_bound_status"
        ] = "unchanged_post_targetmax_source_substitution_bound"
    result["decision"][
        "generation_explanation_status"
    ] = "fixed_recipe_source_frontier_after_resegmentation_second_pass"
