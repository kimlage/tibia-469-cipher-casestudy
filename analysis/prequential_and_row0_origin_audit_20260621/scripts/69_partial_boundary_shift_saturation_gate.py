...
GATE68 = TEST_RESULTS / "68_partial_boundary_shift_second_pass_formula_gate.json"
...
def main() -> None:
    gate68 = load_json(GATE68)
    assert_boundary("partial_boundary_shift_second_pass_formula_gate", gate68)
    ...
    classification = (
        "partial_boundary_shift_saturation_candidate_found"
        if improving
        else "partial_boundary_shift_saturated"
    )
    interpretation = (
        "A further exact-scored partial boundary shift remains after the second "
        "promotion. Another promotion gate would be required."
        if improving
        else "No remaining positive partial local boundary shift improves the "
        "exact active scorer after two promotions."
    )
    result = {
        "schema": "partial_boundary_shift_saturation_gate.v1",
        ...
        "inputs": {
            ...
            "previous_formula_gate": rel(GATE68),
        },
        "scope": {
            "analysis_only": True,
            ...
            "window_family": "post-second-pass two-operation positive partial boundary shifts up to target-max",
        },
        "summary": {
        ...
        "decision": {
            ...
            "partial_boundary_shift_status": (
                "candidate_found_after_second_pass" if improving else "saturated"
            ),
    ...
    (TEST_RESULTS / "69_partial_boundary_shift_saturation_gate.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    (TEST_RESULTS / "69_partial_boundary_shift_saturation_gate.md").write_text(
