...
input_filename = './books_frequency_filtered.md'
output_filename = './books_frequency_filtered_spaces.md'  # Changed to '_spaces'

# Read the input file and process sequences
with open(input_filename, 'r') as infile, open(output_filename, 'w') as outfile:
    outfile.write("| Espaços | Frequência | Comprimento |\n")  # Adicionando colunas para frequência e comprimento
    outfile.write("|----------|------------|-------------|\n")
    
    for line in infile:
        if line.startswith('|'):
            parts = line.strip().split('|')
            if len(parts) >= 4 and parts[1].strip() != "Substring":
                substring = parts[1].strip().replace('`', '')
                
                # Process the substring for spaces
                spaces = process_sequence(substring, known_sequences_sorted)
                
                # Calcular a frequência e o comprimento
                frequency = len(spaces.split())  # Contar o número de espaços
                length = len(spaces)  # Comprimento total da string
                
                # Write spaces, frequency, and length to the output file
                outfile.write(f"| {spaces} | {frequency} | {length} |\n")
        else:
            outfile.write(line)