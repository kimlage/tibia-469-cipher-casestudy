...
| Internal start beam capacity audit | expands the target-free controller beam through x32; coverage improves to `101/150` true coarse sequences and `78/343` generated internal starts, but the paid x32 program still costs `2815.682` bits vs `2811.673` explicit opcount+cutpoint+type declaration after width cost | near-bound capacity clue / not promoted |
