...
| Joint boundary-digit gate | tests the simplest joint target-stream/parser model by emitting `(boundary flag, digit)` pairs under prefix-trained contexts; best nontrivial model `joint_pair_context_order0` is `-29.950` bits versus factorized digit+global-boundary baseline and positive in only `2/5` cells | simple joint pair model rejected |
