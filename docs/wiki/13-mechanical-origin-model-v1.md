...
| Causal event graph program audit | converts the v6 decoder into a graph with seed spans, literal innovation spans, copy intervals, operation spans, endpoint marks, and causal edges; prefix/family macro tests cover `72` split-stream rows, but best macro delta is still `+88.238` bits vs direct event labels, `0` rows are positive, only `2/72` beat shuffled p05, and shuffled literal tape preserves `0/53` chunks | causal graph useful as ledger / macro program not promoted |
