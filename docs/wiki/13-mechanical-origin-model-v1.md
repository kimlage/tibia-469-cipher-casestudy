...
| Literal payload reference subcodec audit | tests whether prior whole-chunk occurrence can replace literal payload with declared references; `38` chunks / `103` digits occurred before and `11` chunks / `48` digits save `30.001` bits before mode cost, but the charged model is `+22.999` bits worse than raw payload | prior-occurrence payload clue rejected as subcodec |
...
| Target-conditioned source collapse audit | if the copied target chunk is granted, source choice mostly collapses to earliest matching source (`200/208`) plus `8` exceptions; earliest+exception costs `58.085` bits, `-174.817` vs oracle rank bits and far outside random rank controls, but it remains target-oracle conditioned | promoted target-conditioned clue / not decoder generator |
| Target chunk dictionary audit | tests whether the missing target stream can be a compact dictionary of exact operation chunks; chunks are nearly unique overall (`256/261`) and copy chunks are `207/208` unique, making the all-chunk dictionary `+32442.167` bits worse than the target-conditioned baseline | exact-chunk dictionary rejected |
| Target chunk signature audit | tests whether exact chunks can be replaced by coarse target-stream signatures; best non-payload signature still leaves exact digits unresolved (`495.649` selector bits after `85` signatures), while payload signatures become near-exact (`251/261` singleton rows for first2/last2) and match same-length random controls | shallow signature generator rejected |
| Target digit process audit | tests source-free digit processes over all `11263` book digits and the `9567` derived-book target digits; `prev2_digits` is strongly predictive under prefix/suffix validation (`2.068621` all70 bpd, `2.108869` derived60 bpd) and beats same-book-histogram shuffled controls, but still leaves arithmetic residual bits and does not explain chunk endpoints | target-digit Markov clue promoted / generator not promoted |
| Target digit boundary audit | tests whether internal skeleton cutpoints align with `prev2_digits` surprisal under prior-book training; real cutpoints have right-surprisal mean `3.808645` vs random p95 `2.293949`, with `88/201` in each book's top surprisal decile, but top-k surprisal selection recovers only `57/201` cutpoints and `0/48` nontrivial books | boundary Markov clue promoted / endpoint generator not promoted |
| Target digit boundary pruning audit | prices the boundary clue as a cutpoint candidate-pruning code: top `0.1` right-surprisal bands cover `86/201` cutpoints, reduce the paid cutpoint atlas by `105.946` bits after q-choice charge, and beat random same-size bands, while prefix-selected suffix savings are positive in `5/5` before q charge and `4/5` after | boundary-pruning clue promoted / endpoint generator not promoted |
...
| Skeleton generation route review | consolidates `8` active route families after the boundary frontier: `0` promoted generator routes, `2` promoted clue/dependency routes, and `5` rejected/weak/deferred routes; the open blocker remains `operation_skeleton`, and the only aligned next route is a joint target-stream/parser or explicit latent-state account | boundary-frontier saturated / route changed |
| Joint target-stream parser first gates | tests first-pass joint parser models; simple `(boundary flag,digit)` pair emission is rejected (`-29.950` bits, `2/5` cells), while sequential boundary hazard state `age_bucket` reduces boundary dependency by `170.175` bits after feature charge; age-only endpoint decoding fails (`9/343`, `0/5` cells), and combined surprisal+hazard decoding improves to `74/343` with `5/5` cells beating random p95 but still has `0` nontrivial exact books | boundary clues promoted / endpoint generator rejected |
| Latent transducer beam gate | opens the joint transducer route: prefix-trained mode/length/boundary/digit costs choose literal spans, copy spans, sources, and boundaries in one beam over future books; it beats random cutpoint p95 in `5/5` cells, gets `224/343` cutpoint hits, saves `675.669` paid cutpoint-correction bits vs atlas, and gets `131/421` source+length hits, but remains teacher-forced by target digits and has `0` nontrivial exact books; a sampled closed-loop rescue ledger forces `15/15` exact books only with `1732` rescues and `21403.967` rescue bits (`3.069669x` raw digit bits) | joint route opened / high external-control / generator not promoted |
| Closed-loop rescue surface audit | classifies the `1732` sampled oracle rescue events against the canonical skeleton after decoding: `1721` are inside copy spans, only `27` are exact internal cutpoints, `82` are near internal cutpoints, and `27` are operation starts | boundary-trigger rescue rejected / missing state is copy-state/content control |
| Copy-state rescue diagnostic | within copy spans, only `16/1721` rescue events arrive via copy while `1705/1721` arrive by literal; sampled canonical sources match `32/32`, raw inventory has a correct prefix for `32/32` and `1063/1240` copy digits, but the pruned candidate set has a correct prefix for `0/32` ops | missing prior material rejected / copy pruning-ranking route opened |
| Copy-candidate ranking frontier | at top-80, simple target-free rankings improve over current source-penalty pruning only weakly: best unique-op policy `longest_recent` keeps `56/1240` copy prefix digits vs current `6/1240`, narrowly beats random digit p95 `55`, but covers only `0.045161` of copy digits | simple chunk ranking not promoted / copy-control hint route opened |
| Copy hint stream lower bound | granting op start, copy type, copy length, and exact prior material, a same-length chunk hint ranked by `frequent_longest` costs `1873.768` bits for `208` copy ops and `9301` copied digits, saving `676.826` bits vs raw source addressing (`2550.594`) and costing `0.060645` of raw copied-digit literal bits | paid copy-control lower bound promoted / generator not promoted |
| Copy hint stream structure gate | prefix-trained rank-bucket contexts over length, occurrence, candidate count, op index, and target-start fail to compress the paid hint stream: direct rank coding costs `3998.858` bits over cutoffs, best bucket+offset costs `5162.759`, and shuffled controls have better p95 (`-1048.351` vs observed `-1163.901` saving) | simple hint-stream structure rejected / copy hint remains external |
| Unified control program audit | consolidates the residual controls into one ledger: `261` ops, `208` copies, `53` literal runs, `266` innovation digits, and `261/261` target starts derived from prior lengths; replacing raw source addresses with copy hints and joint `type:length` control organizes the residual from `5289.866` to `3532.404` bits, but exact books/ops without atlas remain `0/0`; only `previous_op_to_next_control_symbol` is promoted as a non-tautological coupling clue | partial coupling clue / unified generator not promoted |
| Stateful control program audit | tests whether the exact `type:length` stream can be generated or economically coded from book id, book length, remaining length, and previous emitted control; best `remaining_prev_bucket` costs `4600.432` bits across prefix/suffix cutoffs, `1001.211` bits worse than independent exact type+length declaration and worse than shuffled p95 (`-850.701`); greedy exact books are `0`, and Beam20 exact hits are only trivial one-op books | observable state program rejected / exact control stream retained |
| Length innovation factor audit | factors exact length into coarse `type:length_bucket` plus within-bucket residual: independent `op_type + exact_length` costs `1855.639` bits, while `type:length_bucket` stream plus uniform residual costs `1741.641`, saving `113.998`; residual contexts do not promote (`type_bucket` is `-117.997` bits vs uniform residual), so the exact residual tape remains external | factorization clue promoted / residual length innovation retained |
| Coarse control program audit | tests the `type:length_bucket` stream alone under granted per-book op-count; best `op_count` model costs `1512.974` bits across prefix/suffix cutoffs, saves `254.413` bits vs uniform coarse control, and beats same-multiset shuffled p95 (`116.843`); Beam20 recovers nontrivial held-out coarse sequences in every cutoff, but exact residual lengths remain external | coarse-control program candidate / not full generator |
| Book-level coarse length controller audit | integrates `book_length`, latent `op_count`, coarse `type:length_bucket`, and residual composition; true coarse sequence is kept in beam `56/150` times (`13` nontrivial) vs same-multiset p95 `37`, op_count is in beam `120/150`, and residuals under true/corrected coarse sequence become a book-level composition index (`1031.010 -> 665.782`, saving `365.229` bits); integrated latent beam + composition costs `3146.578` vs separated `4478.440` | promoted book-level controller candidate / corrections retained |
| Composition index structure audit | tests whether the exact rank inside the book-level residual composition has reusable prefix-holdout structure; best `count_x_length__quantile10` costs `1211.748` bits versus `1198.420` uniform composition-index bits (`-13.327` saving), with random-rank p95 `-13.273`, edge ranks only `2/48`, and low-half ranks balanced `24/48` | composition index remains external |
| Minimal external tape program audit | builds an executable decoder contract and unified residual tape ledger; paid tapes roundtrip `70/70` books from seed books `0..9` plus `261` ops, with `9992.848` external bits including seed, but macro/template grammar over the IR is `-4942.611` bits worse than separated coarse+composition declaration and yields only `1` nontrivial exact book without sequence atlas | decoder contract + ledger promoted / macro program not promoted |
...
| Executable program frontier synthesis | consolidates the pre-x64 executable decoder state: roundtrip remains `70/70`, but promoted executable tape reductions are `0`; external tape cost is `4358.858` bits excluding seed and `9992.848` including seed, while macro/template (`-4942.611`), source tape removal (`-1609.513`), and book-level controller integration (`-244.881`) are all rejected inside the decoder; later online x64 testing supersedes only the coarse-control part of this frontier | pre-x64 tape-ledger frontier / later coarse-control reduction found |
...
| Bucket chunk-origin beam pilot | first execution of the joint chunk-origin route: granting op start, copy type, previous material, and only coarse length bucket, rank the correct prior chunk; the best policy saves `507.351` bits vs uniform bucket candidates and beats random p05 in `5/5` prefix holdouts, but costs `2649.756` bits vs `1873.768` exact-length copy hint and has only `5/208` top-80 hits | weak chunk-origin clue / not executable |
| Chunk length-prior integration audit | tests the direct rescue `length prior -> same-length copy hint`; full-fit `bucket_opcount_pos` would cost `2436.040` bits vs `2539.550` for composition-index + copy-hint (`-103.509`), but prefix holdout has `0/5` positive-saving cells against uniform feasible length | posthoc length prior / not promoted |
| Markov chunk-content prior audit | tests whether the promoted `prev2` digit-content clue can rank same-length copy chunks; content-first Markov policies beat frequency/recency in `0/5` prefix holdouts and cost `4244.687` aggregate bits vs `3998.858` for frequency/recency (`+245.829`) | content prior rejected for chunk-origin |
| Latent-state route synthesis | consolidates the executable frontier plus joint chunk-origin, bucket-rank, length-prior, Markov-content, observable-stateful, and unified-coupling gates; local length/content/source priors are closed as main route, and the next constructive gate is `latent_nonlocal_state_program_pilot` | representation route changed / no formula promoted |
| Latent nonlocal state program pilot | trains small prefix-frozen HMMs over joint operation tokens; HMM cost is `3204.220` bits vs `5342.667` factorized and `3747.315` unigram, but it beats same-multiset shuffled order p05 in `0/5` cells | multistream coupling clue / nonlocal state not promoted |
| Schedule-state multistream pilot | replaces the hidden HMM state with train-selected visible book/operation schedule states; decoder-visible schedule cost is `3559.712` bits vs `5212.286` factorized (`-1652.574`) and beats factorized in `5/5` cells, but beats same-book shuffled p05 in `0/5`; cheaper remaining/target-position states grant skeleton information | schedule/multistream clue / generator not promoted |
| Book multiset/order factorization audit | decomposes the schedule signal into per-book joint-token bag plus exact order index; the same `3559.712` bits split into `2972.334` bag bits and `587.378` order bits, but bag saving vs global is only `57.222` bits and beats permuted-feature p95 in `0/5` cells | schedule clue localized / book multiset generator not promoted |
| Within-book order program audit | grants the true per-book multiset and tests prefix-trained no-replacement ordering policies; policy order cost is `606.765` bits vs `587.378` uniform order (`+19.387`), with shuffled-train p95 `1/5` and shuffled-test p95 `0/5` | order-index program rejected |
| Sequence mutation program audit | tests a joint sequence-level alternative: edit/mutate a previous training-book operation sequence into the held-out book; selected optimistic edit lower bound costs `4742.368` bits vs `3525.674` sequence unigram (`+1216.694`), shuffled-train p95 `0/5`, and oracle source lower bound is still `+832.040` worse | sequence-mutation generator rejected |
| Generative route frontier synthesis | consolidates the recent operation-token route family: hidden HMM state, schedule state, book multiset/order, within-book order, and sequence mutation all produce `0` promoted generators; next route is `digit_level_content_boundary_transducer`, not another operation-token decomposition | route family closed / next route selected |
| Digit content-boundary transducer audit | first all-position pilot of the selected route: decoder-visible prefix/content feature `suffix4_seen` saves `31.030` bits vs global and beats shuffled-label p05 in `4/5`, but costs `+914.961` bits vs true-count composition and predicts `0` start labels | weak content clue / internal-start generator not promoted |
| Start candidate ranking gate | ranks all internal digit positions into a candidate set for starts; candidate+miss correction cost is `1941.433` bits vs `2063.661` binary start composition (`-122.227`) with `37/343` start hits, but beats random top-K p05 in only `2/5` cells and leaves `306` missed-start corrections | weak candidate-set clue / not promoted |
| Surprisal start candidate gate | adds the promoted digit-boundary surprisal clue to the start-candidate route; decoder-visible surprisal costs `1922.243` bits vs `2063.661` composition and captures `71/343` starts, but beats random top-K p05 in `0/5`; target-conditioned right-surprisal is stronger (`1665.114`, `171/343`, `4/5`) but looks at the next digit | useful diagnostic / executable start program not promoted |
| Lagged-surprisal boundary contract | tests whether right-surprisal can become less oracle-like by marking boundaries one digit late; after charging copied first digits as innovation, cost is `2153.437` bits vs `2063.661` composition (`+89.777`), with `147` copy-start lag taxes and `0/5` random-control cells | lagged boundary rescue rejected |
| Physical topology control signal audit | tests partial public Hellgate bookcase/order metadata against residual control streams in the executable decoder; topology is worse than global coding for coarse control (`-107.149` bits), copy-hint rank bucket (`-102.873`), and op type (`-46.439`), with `0/20` positive splits per target and no permutation-p95 wins | public topology not a control program |
| Book residual mode coupling audit | tests whether remaining book-level residual burdens form a joint latent mode; no-derived-shape joint coding costs `938.211` bits vs `2055.480` independent field bits (`+1117.269` saving), has `20/20` positive splits, and beats shuffled-within-stream p95 (`755.429`); the stricter external-burden-only variant also promotes (`357.235` vs p95 `132.102`) | promoted residual coupling clue / not exact generator |
| Latent book-mode program audit | tests whether that promoted residual mode can be predicted from decoder-visible book state (`book_length`, phase, previous mode); selected program costs `945.479` bits vs `891.772` global mode bits (`-53.707`), has only `2/20` positive splits, and does not beat shuffled-mode p95 | residual mode remains paid label / program not promoted |
| Residual mode header codec audit | tests whether paying the residual mode as a book header reduces exact executable tapes; it saves only `110.682` bits on coarse control, loses `7.887` on literal payload, pays `891.772` header bits, and ends at `10950.680` vs `10161.703` baseline (`-788.977`) with `0/20` positive splits | paid mode header not promoted |
| Residual burden cross-prediction audit | leave-one-field-out modes predict residual burden classes before header (`68.293` literal, `160.010` copy-hint, `166.167` composition bits saved), but all are negative after paying the header (`-787.181`, `-639.919`, `-703.874`) | weak coupling only / no ledger reduction |
| Paid-control context payload codec audit | tests already-paid/derived control fields against residual payload streams without a new header; literal payload digits, copy-hint rank buckets, and composition-index quantile buckets all stay negative (`-61.049`, `-61.147`, `-60.345` bits) and do not beat shuffled-target p95 | paid control context codec not promoted |
| Parser/decoder frontier synthesis audit | consolidates executable tape, paid-control context, branch-choice, beam selector, innovation tape, and joint type:length evidence; closes local codecs/selectors while retaining only weak beam/tape/control clues; next aligned route is target-free internal-operation-start generation with corrections paid | route frontier / generator not promoted |
| Target-free internal start program audit | interprets the book-level `type:length_bucket` controller as a start generator; true coarse sequences remain in beam above controls (`56/150` vs p95 `37`), but only `13/343` internal starts are generated before correction and the paid program costs `3146.578` bits vs `2811.673` explicit opcount+cutpoint+type declaration | weak clue / internal-start program not promoted |
| Internal start beam capacity audit | expands the target-free controller beam through x64; coverage improves to `109/150` true coarse sequences and `98/343` generated internal starts, and the paid x64 program costs `2750.345` bits vs `2811.673` explicit opcount+cutpoint+type declaration after width/rank/correction costs | promoted capacity-ledger reduction candidate / not exact generator |
| Internal start beam control audit | same-multiset shuffled payload controls validate that the x64 beam preferentially preserves the real coarse sequences: real `109` sequence hits and `98` generated internal starts beat control p95 `63` and `56`; the candidate remains a ledger-capacity reduction, not a full generator, because misses and residual corrections are still paid | controlled capacity-ledger candidate / generator not promoted |
| Internal start beam paid-control audit | tests the stricter paid ledger question for the same x64 beam; real coarse-control saving is `818.269` bits, above same-multiset shuffled p95 `466.838`, and real paid coarse bits are `1549.117` vs control p05 `1900.549`; fine residual composition, literal payload, copy/source hints, seed payload, and row0 remain external | paid controlled coarse-control reduction candidate / generator not promoted |
| Online x64 coarse-control program audit | runs the x64 controller once over books `10..69`, training only on previous decoded/corrected books; it gets `37/60` coarse sequences and `78/261` ops without coarse correction, generates `41/201` internal starts, pays `876.412` coarse bits vs the current minimal coarse ledger `935.675` (`+59.263` bits), and beats same-multiset control p95 (`-51.784` vs minimal); coarse+composition falls from `1601.457` to `1542.194` bits | promoted executable coarse-control tape reduction / not full generator |
| Executable v2 residual coupling audit | compiles the executable v2 ledger by replacing uniform coarse control with the online x64 rank/correction tape; external bits excluding seed fall from `4358.858` to `4299.595` (`+59.263`), but online-state composition-index coding is rejected (`1300.041` bits vs `1198.420` uniform, shuffled-train p95 `-9.046`) | executable v2 ledger promoted / composition index remains external |
| Executable v2 remaining-tape coupling audit | tests whether the online x64 state helps code the still-external residual tapes; all targets are negative against global and controls: composition quantile `-20.918` bits, copy-hint rank bucket `-37.807`, literal payload digits `-35.665`; no promoted or weak targets | x64 state localized to coarse control / remaining tapes still external |
| Content-addressed event program audit | replaces separated `composition_index + copy_hint/source + literal_payload` with events that select prior content chunks under the online-x64 coarse bucket; all `208/208` copy sources can be derived after content is selected and `200/208` canonical sources match raw source, but candidate sets are huge (median `31983`, max `1275643`) and the new residual costs `3686.781` bits vs `3423.183` v2 (`+263.598`), with `0/5` holdout splits improving v2 | content-addressed program not promoted / origin-content remains external |
| Event-aligned chunk library audit | restricts content choices to prior operation-boundary spans and short concatenations; best mode explains only `6/208` copy chunks (`202` misses), costs `3322.129` bits vs `3423.183` v2 (`-101.054`), but shuffled completed-book boundaries still save `51.361` bits and copy-rank control is not beaten | event-boundary chunk route not promoted / subchunk origin remains external |
| Source-boundary candidate program audit | derives copy source+length by selecting intervals between decoder-visible boundaries in previous material; best `event_plus_surprisal_top20` derives `29/208` copy intervals, costs `3280.192` bits vs `3423.183` v2 (`-142.991`), improves `5/5` prefix holdouts, and beats random-boundary p95 hits (`29` vs `15`) | promoted partial source-boundary program / `179` copy intervals still external |
| Executable v3 source-boundary program audit | integrates the promoted source-boundary program into the executable decoder contract; roundtrip remains `70/70`, external bits excluding seed fall from `4299.595` to `4156.604`, source-boundary ranks cost `275.077`, fallback copy hints cost `1609.521`, residual composition costs `511.961`, and literal payload remains `883.633` | promoted executable v3 ledger / still partial generator |
| Executable v3 source-boundary robustness audit | charges `4.392` bits to declare one of `7` boundary systems and one of `3` interval policies, leaving full-fit v3 still `-138.598` bits; prefix-only system/policy selection improves `5/5` suffix splits with aggregate `-226.100` bits and selects `event_plus_surprisal_top20` in `4/5` splits | v3 source-boundary reduction robust / still partial |
| Boundary-mark propagation program audit | tests whether source-side boundary marks propagate through copy events and explain future intervals; propagated marks raise derived intervals from `29/208` to `34/208`, but residual cost is `3280.551` vs v3 `3280.192` (`+0.359`), and shuffled propagation gets more hits (`48/208`) while still worsening cost | propagation route not promoted / remaining interval origin still external |
| One-sided source-boundary program audit | extends v3 to intervals with one anchored endpoint while exact length remains in residual composition; endpoint coverage is `29` both-hit, `40` start-only, `56` end-only, `83` neither, and fixed `end_first` reduces v3 residual by `49.465` bits before the `2.000` bit policy declaration; prefix-only policy selection is positive in `5/5` splits | promoted one-sided endpoint program / still partial |
| Executable v4 one-sided boundary audit | integrates the one-sided program into the decoder contract; roundtrip remains `70/70`, external bits excluding seed fall from v3 `4156.604` to `4109.138`, and the v4 classes are `29` two-ended intervals, `56` end-anchored intervals, and `123` fallback copy hints | promoted executable v4 ledger / `83` intervals have no anchored endpoint |
| Literal-span content-origin subprogram audit | follows the v5 source-endpoint frontier and tests whether fallback copy origins can be addressed through already emitted content origins; `literal_span_offset` covers only `11/101` v5 fallback copies, but after paying the model declaration it costs `858.798` bits vs `891.118` copy-hint bits (`-32.320`), has `4/5` positive prefix holdouts, and beats random source-position p05 (`858.798` vs `875.078`) | promoted narrow content-origin subprogram / most fallback copy origins and literal payload remain external |
| Executable v6 literal-span origin audit | integrates the literal-span subprogram into the executable decoder; roundtrip remains `70/70`, external bits excluding seed fall from v5 `4097.333` to v6 `4065.013`, and copy classes become `52` both-endpoint intervals, `55` end-only intervals, `11` literal-span sources, and `90` fallback copy hints | promoted executable v6 ledger / still partial generator |
| Causal event graph program audit | converts the v6 decoder into a `261`-event / `1120`-edge causal graph and tests prefix/family macro programs over exact and high-level event tokens; v6 remains `70/70` with `4065.013` external bits excluding seed, but the best exact-token prefix program is still `+86.439` bits vs the minimal v2 test ledger and produces `0` nontrivial exact books without correction | causal graph useful as ledger / macro program not promoted |
| Innovation lineage basis audit | propagates digit-level lineage from seed/literal atoms through the executable v6 decoder, then tries to address the remaining `90` fallback copy origins in that basis; `55/90` sources are contiguous single-atom intervals, but the paid lineage-address program costs `1022.251` bits vs `779.571` copy-hint bits (`+242.680`), has `0/5` positive prefix splits, and fails randomized-lineage control | lineage provenance useful / innovation-basis program not promoted |
| Lineage signature library audit | tests whether the remaining `90` v6 fallback chunks share a reusable causal-lineage signature library; the best `signature_kind_run_lengths` family needs `63` signatures and costs `1516.786` bits vs `779.571` copy-hint bits (`+737.215`), with `0/5` positive prefix splits and no shuffled-control win | signature-library generator not promoted |
| Residual content-basis program audit | tests a constructive online basis for the same `90` v6 fallback chunks: paid misses enter a reusable content basis, and later chunks may be generated by exact/substring reference; exact reuse hits only `1/90` and costs `+3.419` bits, while substring reuse hits `38/90` but costs `+157.970`, with `0/5` prefix and `0` family positives | content-basis generator not promoted |
| Residual content-fingerprint program audit | tests whether small paid fingerprints of copied content can select the same `90` fallback chunks from prior material after exact length is paid; best `prefix_1` costs `+245.804` bits vs copy-hint and produces `0` unique selections, while longer edge fingerprints reach uniqueness only by becoming much more expensive | content-fingerprint generator not promoted |
| Seed bootstrap copy-surface audit | reopens the largest external field, seed payload, as a target-conditioned surface test: books `0..9` have `1696` raw digits / `5633.990` bits, but greedy previous-copy surface at `min_len=4` covers `1335` digits vs same-multiset shuffled p95 `534`; this promotes a bootstrap-copy surface clue, not an executable generator or seed-authorial claim | seed bootstrap clue / ledger unchanged |
| Seed bootstrap transducer program audit | converts the seed surface clue into a target-free decoder test using seed book lengths, the `361`-digit literal tape from the surface parse, and deterministic context-copy policies; best policy reaches only `55` exact prefix digits, `0/10` exact seed books, and costs `6656.992` bits after correction (`+1023.002` vs raw seed) | seed transducer not promoted |
| Seed bootstrap decision-policy audit | isolates the missing copy/literal decision on the true seed prefix path; simple visible-state rules over suffix repeats and literal-tape pointer reach mean holdout accuracy `0.446`, below shuffled-label p95 `0.847`, with copy recall only about `0.08-0.10` on nonempty splits | seed decision policy not promoted |
| Unified innovation payload audit | unifies the largest raw content fields, seed payload (`1696` digits) plus v6 literal payload (`266` digits), into one paid previous-copy/literal replay ledger; raw cost is `6517.623` bits, best `min_len=8` replay costs `4010.770` after declaration (`-2506.853`), moving the v6 ledger including seed from `9699.003` to `7192.151`; it roundtrips all `63` seed/literal segments, beats same-multiset digit shuffle and segment-order shuffle p05, and has `4/4` positive replay holdouts | promoted payload-ledger reduction / not source-free generator |
| Executable v7 unified innovation payload audit | integrates the unified payload ledger into the decoder contract: one paid innovation stream is replayed, split into seed/literal segments, then fed to the v6 op/source/length decoder; roundtrip remains `70/70`, external bits including seed fall from v6 `9699.003` to v7 `7192.151`, replacing `5633.990` seed bits and `883.633` literal bits with `4010.770` replay bits | promoted executable v7 ledger / innovation replay still target-conditioned |
| Innovation replay policy frontier audit | tests whether the v7 innovation replay can be generated by simple online policies using only emitted material, total stream length, and the residual literal tape; best policy is `literal_only`, reaches only `242/1962` exact prefix digits, and costs `+4906.133` bits after suffix correction vs the v7 replay; segment/event boundary diagnostics are weak (`5/62` starts, `5/62` ends, `0/39` copy source start+end hits) | replay policy not promoted / copy-vs-literal decision remains external |
| Executable v8 innovation-literal Markov audit | integrates a prequential Markov model for the `962` literal digits inside the v7 innovation replay; raw literal cost is `3195.695` bits, selected order `2` costs `2958.453` after declaration, moving total content-included bits from v7 `7192.151` to v8 `6954.909`; same-multiset literal shuffles are beaten at p05 and prefix holdouts are positive in `3/3` splits | promoted executable v8 literal-content ledger / replay schedule still external |
| Executable v9 innovation copy-continuation audit | derives source for copy events that immediately continue the previous copy (`source=previous_source+previous_length`) while paying a combinatorial index over `17` copy-after-copy opportunities; `5` continuations save `51.219` source bits, pay `13.595` pattern bits, preserve `70/70`, and move total content-included bits from v8 `6954.909` to v9 `6917.285`; random-label controls are close, so this is not broad causal evidence | promoted narrow executable source reduction / replay policy still external |
| Post-v9 generator frontier synthesis | recomputes the remaining replay dependencies after v9 and tests the tempting copy-length default (`cap` or `len=9`) as a paid candidate; it is positive but micro/control-close: `23.283` net bits after `41.717` pattern bits and `7.000` declaration bits, below the `50` bit promotion threshold, so no v10 is promoted | v9 retained as frontier / micro length default rejected |
...
[final_target_digit_boundary_peak_audit.md](../../analysis/target_digit_boundary_peak_audit_20260621/reports/final_target_digit_boundary_peak_audit.md),
[target_digit_boundary_peak_gate.md](../../analysis/target_digit_boundary_peak_audit_20260621/reports/test_results/01_target_digit_boundary_peak_gate.md),
[final_target_digit_boundary_island_audit.md](../../analysis/target_digit_boundary_island_audit_20260621/reports/final_target_digit_boundary_island_audit.md),
...
[final_skeleton_generation_route_review.md](../../analysis/skeleton_generation_route_review_20260622/reports/final_skeleton_generation_route_review.md),
[skeleton_generation_route_review.md](../../analysis/skeleton_generation_route_review_20260622/reports/test_results/01_skeleton_generation_route_review.md),
[final_joint_target_stream_parser_audit.md](../../analysis/joint_target_stream_parser_audit_20260622/reports/final_joint_target_stream_parser_audit.md),
[joint_boundary_digit_gate.md](../../analysis/joint_target_stream_parser_audit_20260622/reports/test_results/01_joint_boundary_digit_gate.md),
[boundary_hazard_state_gate.md](../../analysis/joint_target_stream_parser_audit_20260622/reports/test_results/02_boundary_hazard_state_gate.md),
...
- Final unified control program audit:
  [final_unified_control_program_audit.md](../../analysis/unified_control_program_audit_20260622/reports/final_unified_control_program_audit.md)
- Unified residual control ledger:
  [01_unified_residual_control_ledger.md](../../analysis/unified_control_program_audit_20260622/reports/test_results/01_unified_residual_control_ledger.md)
- Unified control program tests:
  [02_unified_control_program_tests.md](../../analysis/unified_control_program_audit_20260622/reports/test_results/02_unified_control_program_tests.md)
- Final stateful control program audit:
  [final_stateful_control_program_audit.md](../../analysis/stateful_control_program_audit_20260622/reports/final_stateful_control_program_audit.md)
- Stateful control program gate:
  [01_stateful_control_program_gate.md](../../analysis/stateful_control_program_audit_20260622/reports/test_results/01_stateful_control_program_gate.md)
- Final length innovation factor audit:
  [final_length_innovation_factor_audit.md](../../analysis/length_innovation_factor_audit_20260622/reports/final_length_innovation_factor_audit.md)
- Length innovation factor gate:
  [01_length_innovation_factor_gate.md](../../analysis/length_innovation_factor_audit_20260622/reports/test_results/01_length_innovation_factor_gate.md)
- Final coarse control program audit:
  [final_coarse_control_program_audit.md](../../analysis/coarse_control_program_audit_20260622/reports/final_coarse_control_program_audit.md)
- Coarse control program gate:
  [01_coarse_control_program_gate.md](../../analysis/coarse_control_program_audit_20260622/reports/test_results/01_coarse_control_program_gate.md)
- Final book-level coarse length controller audit:
  [final_book_level_coarse_length_controller_audit.md](../../analysis/book_level_coarse_length_controller_audit_20260622/reports/final_book_level_coarse_length_controller_audit.md)
- Book-level coarse length controller gate:
  [01_book_level_coarse_length_controller_gate.md](../../analysis/book_level_coarse_length_controller_audit_20260622/reports/test_results/01_book_level_coarse_length_controller_gate.md)
- Final composition index structure audit:
  [final_composition_index_structure_audit.md](../../analysis/composition_index_structure_audit_20260622/reports/final_composition_index_structure_audit.md)
- Composition index structure gate:
  [01_composition_index_structure_gate.md](../../analysis/composition_index_structure_audit_20260622/reports/test_results/01_composition_index_structure_gate.md)
- Final minimal external tape program audit:
  [final_minimal_external_tape_program_audit.md](../../analysis/minimal_external_tape_program_audit_20260622/reports/final_minimal_external_tape_program_audit.md)
- Executable decoder contract:
  [01_executable_decoder_contract.md](../../analysis/minimal_external_tape_program_audit_20260622/reports/test_results/01_executable_decoder_contract.md)
- Unified external tape ledger:
  [02_unified_external_tape_ledger.md](../../analysis/minimal_external_tape_program_audit_20260622/reports/test_results/02_unified_external_tape_ledger.md)
- Macro program gate:
  [03_macro_program_gate.md](../../analysis/minimal_external_tape_program_audit_20260622/reports/test_results/03_macro_program_gate.md)
- Final source tape removal program audit:
  [final_source_tape_removal_program_audit.md](../../analysis/source_tape_removal_program_audit_20260622/reports/final_source_tape_removal_program_audit.md)
- Source tape removal program gate:
  [01_source_tape_removal_program_gate.md](../../analysis/source_tape_removal_program_audit_20260622/reports/test_results/01_source_tape_removal_program_gate.md)
- Final book-level controller program integration audit:
  [final_book_level_controller_program_integration_audit.md](../../analysis/book_level_controller_program_integration_audit_20260622/reports/final_book_level_controller_program_integration_audit.md)
- Book-level controller program integration gate:
  [01_book_level_controller_program_integration_gate.md](../../analysis/book_level_controller_program_integration_audit_20260622/reports/test_results/01_book_level_controller_program_integration_gate.md)
...
- Final joint chunk-origin beam pilot audit:
  [final_joint_chunk_origin_beam_pilot_audit.md](../../analysis/joint_chunk_origin_beam_pilot_audit_20260622/reports/final_joint_chunk_origin_beam_pilot_audit.md)
- Bucket chunk-origin beam pilot:
  [01_bucket_chunk_origin_beam_pilot.md](../../analysis/joint_chunk_origin_beam_pilot_audit_20260622/reports/test_results/01_bucket_chunk_origin_beam_pilot.md)
- Final chunk length-prior integration audit:
  [final_chunk_length_prior_integration_audit.md](../../analysis/chunk_length_prior_integration_audit_20260622/reports/final_chunk_length_prior_integration_audit.md)
- Chunk length-prior integration gate:
  [01_chunk_length_prior_integration_gate.md](../../analysis/chunk_length_prior_integration_audit_20260622/reports/test_results/01_chunk_length_prior_integration_gate.md)
- Final Markov chunk-content prior audit:
  [final_markov_chunk_content_prior_audit.md](../../analysis/markov_chunk_content_prior_audit_20260622/reports/final_markov_chunk_content_prior_audit.md)
- Markov chunk-content prior gate:
  [01_markov_chunk_content_prior_gate.md](../../analysis/markov_chunk_content_prior_audit_20260622/reports/test_results/01_markov_chunk_content_prior_gate.md)
- Final latent-state route synthesis audit:
  [final_latent_state_route_synthesis_audit.md](../../analysis/latent_state_route_synthesis_audit_20260622/reports/final_latent_state_route_synthesis_audit.md)
- Latent-state route synthesis:
  [01_latent_state_route_synthesis.md](../../analysis/latent_state_route_synthesis_audit_20260622/reports/test_results/01_latent_state_route_synthesis.md)
- Final latent nonlocal state program pilot audit:
  [final_latent_nonlocal_state_program_pilot_audit.md](../../analysis/latent_nonlocal_state_program_pilot_audit_20260622/reports/final_latent_nonlocal_state_program_pilot_audit.md)
- Latent nonlocal state program pilot:
  [01_latent_nonlocal_state_program_pilot.md](../../analysis/latent_nonlocal_state_program_pilot_audit_20260622/reports/test_results/01_latent_nonlocal_state_program_pilot.md)
- Final schedule-state multistream pilot audit:
  [final_schedule_state_multistream_pilot_audit.md](../../analysis/schedule_state_multistream_pilot_audit_20260622/reports/final_schedule_state_multistream_pilot_audit.md)
- Schedule-state multistream pilot:
  [01_schedule_state_multistream_pilot.md](../../analysis/schedule_state_multistream_pilot_audit_20260622/reports/test_results/01_schedule_state_multistream_pilot.md)
- Final book multiset/order factorization audit:
  [final_book_multiset_order_factorization_audit.md](../../analysis/book_multiset_order_factorization_audit_20260622/reports/final_book_multiset_order_factorization_audit.md)
- Book multiset/order factorization gate:
  [01_book_multiset_order_factorization_gate.md](../../analysis/book_multiset_order_factorization_audit_20260622/reports/test_results/01_book_multiset_order_factorization_gate.md)
- Final within-book order program audit:
  [final_within_book_order_program_audit.md](../../analysis/within_book_order_program_audit_20260622/reports/final_within_book_order_program_audit.md)
- Within-book order program gate:
  [01_within_book_order_program_gate.md](../../analysis/within_book_order_program_audit_20260622/reports/test_results/01_within_book_order_program_gate.md)
- Final sequence mutation program audit:
  [final_sequence_mutation_program_audit.md](../../analysis/sequence_mutation_program_audit_20260622/reports/final_sequence_mutation_program_audit.md)
- Sequence mutation program gate:
  [01_sequence_mutation_program_gate.md](../../analysis/sequence_mutation_program_audit_20260622/reports/test_results/01_sequence_mutation_program_gate.md)
- Final generative route frontier synthesis audit:
  [final_generative_route_frontier_synthesis_audit.md](../../analysis/generative_route_frontier_synthesis_audit_20260622/reports/final_generative_route_frontier_synthesis_audit.md)
- Generative route frontier synthesis:
  [01_generative_route_frontier_synthesis.md](../../analysis/generative_route_frontier_synthesis_audit_20260622/reports/test_results/01_generative_route_frontier_synthesis.md)
- Final digit content boundary transducer audit:
  [final_digit_content_boundary_transducer_audit.md](../../analysis/digit_content_boundary_transducer_audit_20260622/reports/final_digit_content_boundary_transducer_audit.md)
- All-position boundary transducer gate:
  [01_all_position_boundary_transducer_gate.md](../../analysis/digit_content_boundary_transducer_audit_20260622/reports/test_results/01_all_position_boundary_transducer_gate.md)
- Start candidate ranking gate:
  [02_start_candidate_ranking_gate.md](../../analysis/digit_content_boundary_transducer_audit_20260622/reports/test_results/02_start_candidate_ranking_gate.md)
- Surprisal start candidate gate:
  [03_surprisal_start_candidate_gate.md](../../analysis/digit_content_boundary_transducer_audit_20260622/reports/test_results/03_surprisal_start_candidate_gate.md)
- Lagged-surprisal boundary contract gate:
  [04_lagged_surprisal_boundary_contract_gate.md](../../analysis/digit_content_boundary_transducer_audit_20260622/reports/test_results/04_lagged_surprisal_boundary_contract_gate.md)
- Final physical topology control signal audit:
  [final_physical_topology_control_signal_audit.md](../../analysis/physical_topology_control_signal_audit_20260622/reports/final_physical_topology_control_signal_audit.md)
- Final book residual mode coupling audit:
  [final_book_residual_mode_coupling_audit.md](../../analysis/book_residual_mode_coupling_audit_20260622/reports/final_book_residual_mode_coupling_audit.md)
- Final latent book-mode program audit:
  [final_latent_book_mode_program_audit.md](../../analysis/latent_book_mode_program_audit_20260622/reports/final_latent_book_mode_program_audit.md)
- Final residual mode header codec audit:
  [final_residual_mode_header_codec_audit.md](../../analysis/residual_mode_header_codec_audit_20260622/reports/final_residual_mode_header_codec_audit.md)
- Final residual burden cross-prediction audit:
  [final_residual_burden_cross_prediction_audit.md](../../analysis/residual_burden_cross_prediction_audit_20260622/reports/final_residual_burden_cross_prediction_audit.md)
- Final paid-control context payload codec audit:
  [final_paid_control_context_payload_codec_audit.md](../../analysis/paid_control_context_payload_codec_audit_20260622/reports/final_paid_control_context_payload_codec_audit.md)
- Final parser/decoder frontier synthesis audit:
  [final_parser_decoder_frontier_synthesis_audit.md](../../analysis/parser_decoder_frontier_synthesis_audit_20260622/reports/final_parser_decoder_frontier_synthesis_audit.md)
- Final target-free internal start program audit:
  [final_target_free_internal_start_program_audit.md](../../analysis/target_free_internal_start_program_audit_20260622/reports/final_target_free_internal_start_program_audit.md)
- Final internal start beam capacity audit:
  [final_internal_start_beam_capacity_audit.md](../../analysis/internal_start_beam_capacity_audit_20260622/reports/final_internal_start_beam_capacity_audit.md)
- Final internal start beam control audit:
  [final_internal_start_beam_control_audit.md](../../analysis/internal_start_beam_control_audit_20260622/reports/final_internal_start_beam_control_audit.md)
- Internal start beam control gate:
  [01_internal_start_beam_control_gate.md](../../analysis/internal_start_beam_control_audit_20260622/reports/test_results/01_internal_start_beam_control_gate.md)
- Final internal start beam paid-control audit:
  [final_internal_start_beam_paid_control_audit.md](../../analysis/internal_start_beam_paid_control_audit_20260622/reports/final_internal_start_beam_paid_control_audit.md)
- Internal start beam paid-control gate:
  [01_internal_start_beam_paid_control_gate.md](../../analysis/internal_start_beam_paid_control_audit_20260622/reports/test_results/01_internal_start_beam_paid_control_gate.md)
- Final online x64 coarse-control program audit:
  [final_online_x64_coarse_control_program_audit.md](../../analysis/online_x64_coarse_control_program_audit_20260622/reports/final_online_x64_coarse_control_program_audit.md)
- Online x64 coarse-control program gate:
  [01_online_x64_coarse_control_program_gate.md](../../analysis/online_x64_coarse_control_program_audit_20260622/reports/test_results/01_online_x64_coarse_control_program_gate.md)
- Final executable v2 residual coupling audit:
  [final_executable_v2_residual_coupling_audit.md](../../analysis/executable_v2_residual_coupling_audit_20260622/reports/final_executable_v2_residual_coupling_audit.md)
- Executable v2 residual coupling gate:
  [01_executable_v2_residual_coupling_gate.md](../../analysis/executable_v2_residual_coupling_audit_20260622/reports/test_results/01_executable_v2_residual_coupling_gate.md)
- Final executable v2 remaining-tape coupling audit:
  [final_executable_v2_remaining_tape_coupling_audit.md](../../analysis/executable_v2_remaining_tape_coupling_audit_20260622/reports/final_executable_v2_remaining_tape_coupling_audit.md)
- Executable v2 remaining-tape coupling gate:
  [01_executable_v2_remaining_tape_coupling_gate.md](../../analysis/executable_v2_remaining_tape_coupling_audit_20260622/reports/test_results/01_executable_v2_remaining_tape_coupling_gate.md)
- Final content-addressed event program audit:
  [final_content_addressed_event_program_audit.md](../../analysis/content_addressed_event_program_audit_20260622/reports/final_content_addressed_event_program_audit.md)
- Content-addressed event program gate:
  [01_content_addressed_event_program_gate.md](../../analysis/content_addressed_event_program_audit_20260622/reports/test_results/01_content_addressed_event_program_gate.md)
- Final event-aligned chunk library audit:
  [final_event_aligned_chunk_library_audit.md](../../analysis/event_aligned_chunk_library_audit_20260622/reports/final_event_aligned_chunk_library_audit.md)
- Event-aligned chunk library gate:
  [01_event_aligned_chunk_library_gate.md](../../analysis/event_aligned_chunk_library_audit_20260622/reports/test_results/01_event_aligned_chunk_library_gate.md)
- Final source-boundary candidate program audit:
  [final_source_boundary_candidate_program_audit.md](../../analysis/source_boundary_candidate_program_audit_20260622/reports/final_source_boundary_candidate_program_audit.md)
- Source-boundary candidate program gate:
  [01_source_boundary_candidate_program_gate.md](../../analysis/source_boundary_candidate_program_audit_20260622/reports/test_results/01_source_boundary_candidate_program_gate.md)
- Final executable v3 source-boundary program audit:
  [final_executable_v3_source_boundary_program_audit.md](../../analysis/executable_v3_source_boundary_program_audit_20260622/reports/final_executable_v3_source_boundary_program_audit.md)
- Executable v3 source-boundary program gate:
  [01_executable_v3_source_boundary_program_gate.md](../../analysis/executable_v3_source_boundary_program_audit_20260622/reports/test_results/01_executable_v3_source_boundary_program_gate.md)
- Final executable v3 source-boundary robustness audit:
  [final_executable_v3_source_boundary_robustness_audit.md](../../analysis/executable_v3_source_boundary_robustness_audit_20260622/reports/final_executable_v3_source_boundary_robustness_audit.md)
- Executable v3 source-boundary robustness gate:
  [01_executable_v3_source_boundary_robustness_gate.md](../../analysis/executable_v3_source_boundary_robustness_audit_20260622/reports/test_results/01_executable_v3_source_boundary_robustness_gate.md)
- Final boundary-mark propagation program audit:
  [final_boundary_mark_propagation_program_audit.md](../../analysis/boundary_mark_propagation_program_audit_20260622/reports/final_boundary_mark_propagation_program_audit.md)
- Boundary-mark propagation program gate:
  [01_boundary_mark_propagation_program_gate.md](../../analysis/boundary_mark_propagation_program_audit_20260622/reports/test_results/01_boundary_mark_propagation_program_gate.md)
- Final one-sided source-boundary program audit:
  [final_one_sided_source_boundary_program_audit.md](../../analysis/one_sided_source_boundary_program_audit_20260622/reports/final_one_sided_source_boundary_program_audit.md)
- One-sided source-boundary program gate:
  [01_one_sided_source_boundary_program_gate.md](../../analysis/one_sided_source_boundary_program_audit_20260622/reports/test_results/01_one_sided_source_boundary_program_gate.md)
...
- Final joint content-origin program audit:
  [final_joint_content_origin_program_audit.md](../../analysis/joint_content_origin_program_audit_20260622/reports/final_joint_content_origin_program_audit.md)
- Joint content-origin program gate:
  [01_joint_content_origin_program_gate.md](../../analysis/joint_content_origin_program_audit_20260622/reports/test_results/01_joint_content_origin_program_gate.md)
- Final executable v6 literal-span origin audit:
  [final_executable_v6_literal_span_origin_audit.md](../../analysis/executable_v6_literal_span_origin_audit_20260622/reports/final_executable_v6_literal_span_origin_audit.md)
- Executable v6 literal-span origin gate:
  [01_executable_v6_literal_span_origin_gate.md](../../analysis/executable_v6_literal_span_origin_audit_20260622/reports/test_results/01_executable_v6_literal_span_origin_gate.md)
- Final causal event graph program audit:
  [final_causal_event_graph_program_audit.md](../../analysis/causal_event_graph_program_audit_20260622/reports/final_causal_event_graph_program_audit.md)
- Causal event graph program gate:
  [01_causal_event_graph_program_gate.md](../../analysis/causal_event_graph_program_audit_20260622/reports/test_results/01_causal_event_graph_program_gate.md)
- Final innovation lineage basis audit:
  [final_innovation_lineage_basis_audit.md](../../analysis/innovation_lineage_basis_audit_20260622/reports/final_innovation_lineage_basis_audit.md)
- Innovation lineage basis gate:
  [01_innovation_lineage_basis_gate.md](../../analysis/innovation_lineage_basis_audit_20260622/reports/test_results/01_innovation_lineage_basis_gate.md)
...
- Shared literal-length tape gate:
  [01_shared_literal_length_tape_gate.md](../../analysis/shared_innovation_tape_audit_20260622/reports/test_results/01_shared_literal_length_tape_gate.md)
- Final unified innovation payload audit:
  [final_unified_innovation_payload_audit.md](../../analysis/unified_innovation_payload_audit_20260622/reports/final_unified_innovation_payload_audit.md)
- Unified innovation payload gate:
  [01_unified_innovation_payload_gate.md](../../analysis/unified_innovation_payload_audit_20260622/reports/test_results/01_unified_innovation_payload_gate.md)
- Final executable v7 unified innovation payload audit:
  [final_executable_v7_unified_innovation_payload_audit.md](../../analysis/executable_v7_unified_innovation_payload_audit_20260622/reports/final_executable_v7_unified_innovation_payload_audit.md)
- Executable v7 unified innovation payload gate:
  [01_executable_v7_unified_innovation_payload_gate.md](../../analysis/executable_v7_unified_innovation_payload_audit_20260622/reports/test_results/01_executable_v7_unified_innovation_payload_gate.md)
- Final innovation replay policy frontier audit:
  [final_innovation_replay_policy_frontier_audit.md](../../analysis/innovation_replay_policy_frontier_audit_20260622/reports/final_innovation_replay_policy_frontier_audit.md)
- Innovation replay policy frontier gate:
  [01_innovation_replay_policy_frontier_gate.md](../../analysis/innovation_replay_policy_frontier_audit_20260622/reports/test_results/01_innovation_replay_policy_frontier_gate.md)
- Final executable v8 innovation-literal Markov audit:
  [final_executable_v8_innovation_literal_markov_audit.md](../../analysis/executable_v8_innovation_literal_markov_audit_20260622/reports/final_executable_v8_innovation_literal_markov_audit.md)
- Executable v8 innovation-literal Markov gate:
  [01_executable_v8_innovation_literal_markov_gate.md](../../analysis/executable_v8_innovation_literal_markov_audit_20260622/reports/test_results/01_executable_v8_innovation_literal_markov_gate.md)
- Final executable v9 innovation copy-continuation audit:
  [final_executable_v9_innovation_copy_continuation_audit.md](../../analysis/executable_v9_innovation_copy_continuation_audit_20260622/reports/final_executable_v9_innovation_copy_continuation_audit.md)
- Executable v9 innovation copy-continuation gate:
  [01_executable_v9_innovation_copy_continuation_gate.md](../../analysis/executable_v9_innovation_copy_continuation_audit_20260622/reports/test_results/01_executable_v9_innovation_copy_continuation_gate.md)
- Final post-v9 generator frontier synthesis audit:
  [final_post_v9_generator_frontier_synthesis_audit.md](../../analysis/post_v9_generator_frontier_synthesis_audit_20260622/reports/final_post_v9_generator_frontier_synthesis_audit.md)
- Post-v9 generator frontier synthesis gate:
  [01_post_v9_generator_frontier_synthesis.md](../../analysis/post_v9_generator_frontier_synthesis_audit_20260622/reports/test_results/01_post_v9_generator_frontier_synthesis.md)
