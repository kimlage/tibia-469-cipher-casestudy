...
| Joint target-stream parser first gates | tests first-pass joint parser models; simple `(boundary flag,digit)` pair emission is rejected (`-29.950` bits, `2/5` cells), while sequential boundary hazard state `age_bucket` reduces boundary dependency by `170.175` bits after feature charge, is positive in `5/5` prefix cells, and beats same-count random p95; however, an endpoint decoder granted true op-count gets only `9/343` cutpoint hits, `0/5` cells beating random p95, and `0` nontrivial exact books | hazard-state clue promoted / endpoint decoder rejected |
