...
An operation shape-count generation audit then asks whether the coarse pair
`(op_count, literal_count)` is generated by the same source-free surface. The
...
A literal payload generation audit then attacks the next external dependency
after granting that exact skeleton. The remaining `53` chunks / `266` literal
digits are not generated by simple source-free contexts: the best full-fit
context carries `222` payload digits in its table, is `+44.588` bits worse
than raw uniform literal payload after paid costs, and gets `0/5` prefix/
holdout cells with any exact chunk. Literal payload therefore remains external.
A literal-payload reference subcodec audit then tests whether the whole chunks
already seen in emitted text can replace raw payload. The recurrence is real:
`38` chunks / `103` digits occurred before, and `11` chunks / `48` digits save
`30.001` bits before mode cost. It still fails as a decodable subcodec because
the charged mode/source model is `+22.999` bits worse than raw payload.
A copy-source generation audit then tests the other residual dependency after
granting exact skeleton and literal payload. The target-aware matching controls
can copy correctly (`208/208`) but require knowing the target chunk.
Decoder-visible policies fail (`8/208` best chunk hits; `1/60` exact books), and the
